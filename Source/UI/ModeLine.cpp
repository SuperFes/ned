...
ModeLine::ModeLine(const ActiveBuffer& activeBuffer, const editor::Mode& mode, const Theme& theme) : Widget{ox::FocusPolicy::None, ox::SizePolicy::flex()}, activeBuffer_(activeBuffer), mode_(mode), theme_(theme) {
}

void ModeLine::paint(ox::Canvas c) {
    ...
    for (int x = 0; x < c.size.width; ++x) {
        const char32_t symbol = (static_cast<std::size_t>(x) < text.size())
                                    ? static_cast<char32_t>(static_cast<unsigned char>(text[static_cast<std::size_t>(x)]))
                                    : U' ';
        ...
        const ox::TrueColor background = ox::gradient_blend(theme_.modeLineGradientStart, theme_.modeLineGradientEnd, percent);
        const ox::Brush     brush{.background = background, .foreground = theme_.modeLineForeground};

        c[{.x = x, .y = 0}] = ox::Glyph{.symbol = symbol, .brush = brush};
    }
}
