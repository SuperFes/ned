...
ModeLine::ModeLine(const ActiveBuffer& activeBuffer, const editor::Mode& mode, const Theme& theme)
...
void ModeLine::Paint(Canvas c) {
    ...
    for (int x = 0; x < c.size().width; ++x) {
        ...
        cell.background_color =
            ftxui::Color::Interpolate(percent, theme_.modeLineGradientStart.ToFtxui(), theme_.modeLineGradientEnd.ToFtxui());
        cell.foreground_color = theme_.modeLineForeground.ToFtxui();
    }
}
