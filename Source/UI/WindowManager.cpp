...
Pane::Pane(text::Buffer& buffer, text::KillRing& killRing, editor::RegisterTable& registers,
           ...
           std::function<void(text::Buffer&)>              onBufferClosed) : activeBuffer_(buffer), mode_(std::move(mode)),
                                                                dispatcher_(registry, editor::KeymapStack({&janetKeymap, &mode_.keymap, &globalKeymap},
                                                                                                          {"User (init.janet)", "Major mode", "Global"})),
...
void WindowManager::SetOnNewTerminalRequest(std::function<void()> onNewTerminal) {
    onNewTerminalRequest_ = std::move(onNewTerminal);
    for (Pane* pane : Leaves()) {
        pane->Buffer().SetOnNewTerminalRequest(onNewTerminalRequest_);
    }
}
...
void WindowManager::SetOnContextMenuChanged(std::function<void(std::optional<ListPopupModel>)> onContextMenuChanged) {
    onContextMenuChanged_ = std::move(onContextMenuChanged);
    for (Pane* pane : Leaves()) {
        pane->Buffer().SetOnContextMenuChanged(onContextMenuChanged_);
    }
}
...
void WindowManager::ActivateContextMenuAt(std::size_t index) {
    if (Pane* pane = FocusedPane()) {
        pane->Buffer().ActivateContextMenuAt(index);
    }
}
...
void WindowManager::ScrollCompletionPopup(int steps) {
    if (Pane* pane = FocusedPane()) {
        pane->Buffer().ScrollCompletionPopup(steps);
    }
}
