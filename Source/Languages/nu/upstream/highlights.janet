...
# ned: upstream quantifies the delimiter alternation (`]*`), outside
# QueryMatcher's measured scope; each delimiter child is captured either way.
(path
  [
    "."
    "?"
...
# ned: upstream's optional "$"/"...$" sigil captures are dropped (quantified
# bare tokens); the variable body keeps its classes.
(val_variable
  [
    (identifier) @variable
    "in" @special
...
# ned: upstream's `(comment)+` run before a definition is spelled as the
# one comment adjacent to it (a '+' run is outside QueryMatcher's scope).
