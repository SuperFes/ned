# ned-adapted (cmake/diff/sql precedent): upstream's highlights.scm quantifies
# a bare string token -- `"?"? @punctuation.delimiter` on a formal parameter's
# optional default marker -- which QueryMatcher's census-measured scope
# deliberately excludes (quantifiers on strings/alternations are a stated
# non-goal, not merely unmeasured). One clause dropped; everything else below
