...
# ned: upstream quantifies the bare "=" token and its default value in one
# pattern ("="? (identifier)?), outside QueryMatcher's measured scope; the
# two shapes it means are spelled out.
...
# ned: upstream's trailing "="? is dropped (a quantified bare token is
# outside QueryMatcher's measured scope; the match is the same).
...
# ned: upstream's trailing "="? is dropped, as above.
