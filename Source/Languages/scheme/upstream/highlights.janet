...
# ned: upstream's `(symbol)+ @variable` run between the brackets is
# spelled as one symbol child; each is captured either way.
(list
  .
  "["
  (symbol) @variable
...
# ned: upstream's `_*` capture (a quantified bare wildcard) is spelled as a
# plain `(_)` child throughout this block: every child is captured
# either way.
# 'atom
(quote
  _ @constant)
