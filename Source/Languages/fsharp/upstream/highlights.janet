...
(member_signature
  ...
  # ned: upstream quantifies the bare "*" and "?" tokens ("*"*, "?"?),
  # outside QueryMatcher's measured scope; the optional-token captures are
  # dropped, the parameter name kept.
  (curried_spec
    (arguments_spec
      (argument_spec
        (argument_name_spec
...
# ned: upstream's `(identifier)+ @variable.member . (identifier)` (every
# identifier but the last) is spelled without the '+' run: an identifier
# that has a later identifier sibling is the same set.
((long_identifier
