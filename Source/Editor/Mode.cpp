...
namespace {
    ...
    const std::unordered_map<std::string_view, SyntaxClass>& CaptureTable() {
        static const std::unordered_map<std::string_view, SyntaxClass> table = {
            ...
            // fallback -- the markdown.highlight escape's own heading-level pass
            // (Languages/Markdown.cpp) always overrides it with a real HeadlineLevel1/2/3
            ...
            // capture it; Injection.cpp appends a small supplemental
    ...
    // language-scoped-capture-rules follow-up: a language's own defaults for
    // a capture name that means something different in that one grammar
    // than CaptureTable()'s shared mapping gives it, read from its
    // definition (LanguageDefinition::captureClasses -- Markdown's
    // "punctuation.special" is a MarkupMarker there and Punctuation
    // everywhere else). Keyed by the language key SyntaxClassForCapture's
    // callers pass explicitly; consulted only for that language, so an
    // entry has zero effect on any other language's use of the same name.
    std::optional<SyntaxClass> BuiltinLanguageClassForCapture(std::string_view language, std::string_view captureName) {
        ...
        for (const auto& [capture, cls] : definition->captureClasses) {
            if (capture == captureName) {
                return cls;
            }
        }
        return std::nullopt;
...
SyntaxClass SyntaxClassForCapture(std::string_view captureName, std::string_view language) {
    ...
    // from a headline's own star count, shared by the Org and Markdown
    // highlight escapes. Cycles every 3 (real Org itself cycles
...
// Every bundled *Mode() below is its definition (BundledLanguages.cpp) built
// through the one path -- kept as named functions so a caller (tests, the
// factory table) can ask for a language without knowing the registry.
namespace {
    Mode BundledMode(std::string_view name) {
        return ModeFromDefinition(*BundledLanguage(name));
    }
} // namespace

Mode FundamentalMode() {
    return BundledMode("fundamental");
}
...
std::optional<LocalCaptureKind> LocalCaptureKindFromCaptureName(std::string_view captureName, std::string* qualifier) {
    ...
    if (captureName == "local.scope") {
        return LocalCaptureKind::Scope;
    }
    if (captureName == "local.reference") {
        return LocalCaptureKind::Reference;
    }
    constexpr std::string_view kDefinition = "local.definition";
    if (captureName == kDefinition) {
        return LocalCaptureKind::Definition;
    }
    // "local.definition.<qualifier>" -- the '.' must be there, so that a
    // hypothetical "local.definitions" capture isn't read as a definition
    // named "s".
    if (captureName.size() > kDefinition.size() + 1 && captureName.starts_with(kDefinition) &&
        captureName[kDefinition.size()] == '.') {
        if (qualifier != nullptr) {
            *qualifier = std::string(captureName.substr(kDefinition.size() + 1));
        }
        return LocalCaptureKind::Definition;
    }
    return std::nullopt;
...
Mode TreeSitterModeFromLanguage(std::string name, const treesitter::Language& language, const TreeSitterQuerySources& queries,
    ...
    // already true of queries.folds below. An empty queries.highlights leaves
    // mode.highlight a default-constructed, empty std::function, the exact
    // "no highlighting" signal BufferView already checks for.
    // An injections query rides the same parse: the embedded-language spans
    // (Injection.h) are appended after the host's own so they win over
    // whatever the host query captured across that range -- <script>
    ...
    // embedded-language-documents follow-up: the injection matches as byte
    // ranges + language identity rather than coloured spans, for
    // Editor/EmbeddedDocuments.h to sync to real LSP servers. Built whenever
    ...
    if (!queries.tests.empty()) {
        ...
        testDiscovery        = [parser, testQuery, sharedParse](std::string_view bufferText) -> std::vector<TestMarker> {
            ...
            for (Definition& definition : definitions) {
                ...
                // A definition whose body the grammar leaves as a sibling (an
                // unexpanded C++ TEST_CASE macro) is widened by that
                // language's own escape, not here -- see Languages/CLike.cpp.
                markers.push_back({.startByte = definition.start, .endByte = definition.end, .name = std::move(definition.name)});
            }
            std::sort(markers.begin(), markers.end(),
    ...
    // text hasn't changed since the last repaint gets the cached tree for
...
Mode JanetMode() {
    return BundledMode("janet");
}

Mode JsonMode() {
    return BundledMode("json");
}

Mode CMode() {
    return BundledMode("c");
}

Mode CppMode() {
    return BundledMode("cpp");
}

Mode PhpMode() {
    return BundledMode("php");
}

Mode JavaScriptMode() {
    return BundledMode("javascript");
}

Mode TypeScriptMode() {
    return BundledMode("typescript");
}

Mode TsxMode() {
    return BundledMode("tsx");
}

Mode HtmlMode() {
    return BundledMode("html");
}

Mode CssMode() {
    return BundledMode("css");
}

Mode PythonMode() {
    return BundledMode("python");
}

Mode BashMode() {
    return BundledMode("bash");
}

Mode FishMode() {
    return BundledMode("fish");
}

Mode XmlMode() {
    return BundledMode("xml");
}

Mode RustMode() {
    return BundledMode("rust");
}

Mode GoMode() {
    return BundledMode("go");
}

Mode CSharpMode() {
    return BundledMode("csharp");
}

Mode JavaMode() {
    return BundledMode("java");
}

Mode KotlinMode() {
    return BundledMode("kotlin");
}

Mode YamlMode() {
    return BundledMode("yaml");
}
...
Mode JankMode() {
    return BundledMode("jank");
}

Mode MarkdownMode() {
    return BundledMode("markdown");
}

Mode OrgMode() {
    return BundledMode("org");
}
