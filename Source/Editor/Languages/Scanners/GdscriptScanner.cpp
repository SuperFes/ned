// The gdscript external scanner, ported from https://github.com/PrestonKnopp/tree-sitter-gdscript (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::gdscript {
 ...
 */
static bool lookahead_string(Lexer *lexer, const char *string) {
 ...
 */
static inline bool skip_whitespace(Lexer *lexer, uint32_t *indent_length, bool *found_end_of_line) {
    if (lexer->lookahead == '\n') {
...
static inline void handle_quote(Lexer *lexer, Delimiter *delimiter, char quote) {
    ...
    if (valid_symbols[STRING_START] ||
        ...
        if (end_character(&delimiter)) {
            ...
            if (is_node_path(&delimiter)) {
                lexer->resultSymbol = NODE_PATH_START;
            } else if (is_name(&delimiter)) {
                lexer->resultSymbol = STRING_NAME_START;
            } else {
...
void *Create() {
    ...
    scanner->indents = scanner_calloc(1, sizeof(indent_vec));
    scanner->delimiters = scanner_calloc(1, sizeof(delimiter_vec));
