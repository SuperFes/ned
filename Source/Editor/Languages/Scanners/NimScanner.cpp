// The nim external scanner, ported from https://github.com/alaviss/tree-sitter-nim (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::nim {
    ...
    struct indent_vec* self, int32_t size)
{
  ...
  if (size != self->capacity) {
    indent_value* new_data = scanner_realloc(self->data, size);
    if (!new_data) {
      return -1;
    }
    self->data = new_data;
...
static void valid_tokens_debug(struct valid_tokens self)
{
  if (debug_mode) {
    ...
    for (int i = TOKEN_TYPE_START; i < TOKEN_TYPE_LEN; i++) {
      if (valid_tokens_test(self, (enum token_type)i)) {
...
{
  struct state* result = scanner_calloc(1, sizeof(struct state));
  if (!result) {
    return NULL;
  }
  return result;
...
{
  self->_lexer->markEnd(self->_lexer);
}
    ...
    struct context* self, enum token_type type)
{
  ...
  self->_lexer->resultSymbol = (Symbol)type;
  return true;
}
...
static unsigned SerializeBytes(
    void* payload_, uint8_t* buffer)
{
    VoidPtr payload{payload_};
  if (!payload || !buffer) {
  ...
  return state_serialize(
      (struct state*)payload, buffer, kSerializationBufferSize);
}

static void DeserializeBytes(
    void* payload_, const uint8_t* buffer, unsigned length)
{
    VoidPtr payload{payload_};
  if (!payload) {
...
static bool Scan(
    void* payload_, Lexer* lexer, const bool* valid_tokens)
{
    VoidPtr payload{payload_};
...
// The vtable carries char buffers; upstream serializes bytes.
static unsigned Serialize(void* payload, char* buffer) {
    return SerializeBytes(payload, reinterpret_cast<uint8_t*>(buffer));
}
static void Deserialize(void* payload, const char* buffer, unsigned length) {
    DeserializeBytes(payload, reinterpret_cast<const uint8_t*>(buffer), length);
}
