// The markdown external scanner, ported from https://github.com/tree-sitter-grammars/tree-sitter-markdown (src/scanner.c, MIT
...
static void push_block(Scanner* s, Block b) {
    if (s->open_blocks.size == s->open_blocks.capacity) {
        ...
        VoidPtr tmp = scanner_realloc(s->open_blocks.items,
...
static void mark_end(Scanner* s, Lexer* lexer) {
    if (!s->simulate) {
        lexer->markEnd(lexer);
    }
}
...
static size_t advance(Scanner* s, Lexer* lexer) {
    size_t size = 1;
    if (lexer->lookahead == '\t') {
        size      = 4 - s->column;
        s->column = 0;
...
static bool match(Scanner* s, Lexer* lexer, Block block) {
    switch (block) {
...
static bool parse_fenced_code_block(Scanner* s, const char delimiter,
                                    Lexer* lexer, const bool* valid_symbols) {
...
static bool parse_star(Scanner* s, Lexer* lexer, const bool* valid_symbols) {
    advance(s, lexer);
    mark_end(s, lexer);
    ...
    for (;;) {
        if (lexer->lookahead == '*') {
            if (star_count == 1 && extra_indentation >= 1 &&
                ...
                // later that this is a list item.markEnd(s, lexer);
            }
            star_count++;
            advance(s, lexer);
        }
    ...
    if (valid_symbols[THEMATIC_BREAK] && thematic_break && s->indentation < 4) {
        ...
        lexer->resultSymbol = THEMATIC_BREAK;
        mark_end(s, lexer);
        s->indentation = 0;
        return true;
    }
...
static bool parse_thematic_break_underscore(Scanner* s, Lexer* lexer,
...
static bool parse_block_quote(Scanner* s, Lexer* lexer,
                              const bool* valid_symbols) {
    if (valid_symbols[BLOCK_QUOTE_START]) {
        ...
        lexer->resultSymbol = BLOCK_QUOTE_START;
        if (!s->simulate)
...
static bool parse_atx_heading(Scanner* s, Lexer* lexer,
                              const bool* valid_symbols) {
    if (valid_symbols[ATX_H1_MARKER] && s->indentation <= 3) {
        ...
        if (level <= 6 &&
            (lexer->lookahead == ' ' || lexer->lookahead == '\t' ||
             lexer->lookahead == '\n' || lexer->lookahead == '\r')) {
            lexer->resultSymbol = ATX_H1_MARKER + (level - 1);
            s->indentation      = 0;
            mark_end(s, lexer);
            return true;
        }
...
static bool parse_setext_underline(Scanner* s, Lexer* lexer,
                                   const bool* valid_symbols) {
    if (valid_symbols[SETEXT_H1_UNDERLINE] &&
        ...
        if (lexer->lookahead == '\n' || lexer->lookahead == '\r') {
            lexer->resultSymbol = SETEXT_H1_UNDERLINE;
            mark_end(s, lexer);
            return true;
        }
    }
...
static bool parse_plus(Scanner* s, Lexer* lexer, const bool* valid_symbols) {
    if (s->indentation <= 3 &&
        ...
        if (valid_symbols[PLUS_METADATA] && lexer->lookahead == '+') {
            ...
            for (;;) {
                ...
                if (plus_count == 3) {
                    ...
                    if (lexer->lookahead == '\r' || lexer->lookahead == '\n') {
                        ...
                        mark_end(s, lexer);
                        lexer->resultSymbol = PLUS_METADATA;
                        return true;
                    }
                }
            ...
            if (extra_indentation >= 1 &&
                (dont_interrupt ? valid_symbols[LIST_MARKER_PLUS_DONT_INTERRUPT]
                                : valid_symbols[LIST_MARKER_PLUS])) {
                lexer->resultSymbol = dont_interrupt
                                          ? LIST_MARKER_PLUS_DONT_INTERRUPT
                                          : LIST_MARKER_PLUS;
                extra_indentation--;
                if (extra_indentation <= 3) {
...
static bool parse_ordered_list_marker(Scanner* s, Lexer* lexer,
                                      const bool* valid_symbols) {
    if (s->indentation <= 3 &&
...
static bool parse_minus(Scanner* s, Lexer* lexer, const bool* valid_symbols) {
    if (s->indentation <= 3 &&
        ...
        if (valid_symbols[SETEXT_H2_UNDERLINE] && underline) {
            lexer->resultSymbol = SETEXT_H2_UNDERLINE;
            mark_end(s, lexer);
            s->indentation = 0;
            success        = true;
        }
            ...
            for (;;) {
                ...
                if (minus_count == 3) {
                    ...
                    if (lexer->lookahead == '\r' || lexer->lookahead == '\n') {
                        ...
                        mark_end(s, lexer);
                        lexer->resultSymbol = MINUS_METADATA;
                        return true;
                    }
                }
                // otherwise consume rest of line
...
static bool parse_html_block(Scanner* s, Lexer* lexer,
                             const bool* valid_symbols) {
    ...
    if (lexer->lookahead == '?' && valid_symbols[HTML_BLOCK_3_START]) {
        advance(s, lexer);
        lexer->resultSymbol = HTML_BLOCK_3_START;
        if (!s->simulate)
    ...
    if (lexer->lookahead == '!') {
        ...
        if (lexer->lookahead == '-') {
            ...
            if (lexer->lookahead == '-' && valid_symbols[HTML_BLOCK_2_START]) {
                advance(s, lexer);
                lexer->resultSymbol = HTML_BLOCK_2_START;
                if (!s->simulate)
                    push_block(s, ANONYMOUS);
                return true;
            }
                 ...
                 valid_symbols[HTML_BLOCK_4_START]) {
            advance(s, lexer);
            lexer->resultSymbol = HTML_BLOCK_4_START;
            if (!s->simulate)
                push_block(s, ANONYMOUS);
            return true;
        }
                        ...
                        if (lexer->lookahead == 'T') {
                            ...
                            if (lexer->lookahead == 'A') {
                                ...
                                if (lexer->lookahead == '[' &&
                                    valid_symbols[HTML_BLOCK_5_START]) {
                                    advance(s, lexer);
                                    lexer->resultSymbol = HTML_BLOCK_5_START;
                                    if (!s->simulate)
                                        push_block(s, ANONYMOUS);
                                    return true;
                                }
                ...
                if (strcmp(name, HTML_TAG_NAMES_RULE_1[i]) == 0) {
                    if (starting_slash) {
                        if (valid_symbols[HTML_BLOCK_1_END]) {
                            lexer->resultSymbol = HTML_BLOCK_1_END;
                            return true;
                        }
                    }
                    else if (valid_symbols[HTML_BLOCK_1_START]) {
                        lexer->resultSymbol = HTML_BLOCK_1_START;
                        if (!s->simulate)
                            push_block(s, ANONYMOUS);
                        return true;
                    }
        ...
        if (next_symbol_valid || tag_closed) {
            ...
            for (size_t i = 0; i < NUM_HTML_TAG_NAMES_RULE_7; i++) {
                if (strcmp(name, HTML_TAG_NAMES_RULE_7[i]) == 0 &&
                    valid_symbols[HTML_BLOCK_6_START]) {
                    lexer->resultSymbol = HTML_BLOCK_6_START;
                    if (!s->simulate)
                        push_block(s, ANONYMOUS);
                    return true;
                }
    ...
    if (lexer->lookahead == '\r' || lexer->lookahead == '\n') {
        lexer->resultSymbol = HTML_BLOCK_7_START;
        if (!s->simulate)
            push_block(s, ANONYMOUS);
        return true;
    }
...
static bool parse_pipe_table(Scanner* s, Lexer* lexer,
                             const bool* valid_symbols) {
    ...
    lexer->resultSymbol = PIPE_TABLE_START;
    return true;
}
...
static bool scan(Scanner* s, Lexer* lexer, const bool* valid_symbols) {
    ...
    if (valid_symbols[CLOSE_BLOCK]) {
        ...
        lexer->resultSymbol = CLOSE_BLOCK;
        return true;
    }
    ...
    if (lexer->eof(lexer)) {
        if (valid_symbols[TOKEN_EOF]) {
            lexer->resultSymbol = TOKEN_EOF;
            return true;
        }
        if (s->open_blocks.size > 0) {
            lexer->resultSymbol = BLOCK_CLOSE;
            if (!s->simulate)
                pop_block(s);
            return true;
        }
    ...
    if (!(s->state & STATE_MATCHING)) {
        ...
        if (valid_symbols[INDENTED_CHUNK_START] &&
            ...
            if (s->indentation >= 4 && lexer->lookahead != '\n' &&
                lexer->lookahead != '\r') {
                lexer->resultSymbol = INDENTED_CHUNK_START;
                if (!s->simulate)
        ...
        switch (lexer->lookahead) {
            ...
            case '\n':
                if (valid_symbols[BLANK_LINE_START]) {
                    ...
                    lexer->resultSymbol = BLANK_LINE_START;
                    return true;
                }
                break;
            case '`':
        ...
        if (partial_success) {
            ...
            lexer->resultSymbol = BLOCK_CONTINUATION;
            return true;
        }
    ...
    if ((valid_symbols[LINE_ENDING] || valid_symbols[SOFT_LINE_ENDING] ||
            ...
            if (!lexer->eof(lexer) &&
                ...
                if (valid_symbols[PIPE_TABLE_LINE_ENDING]) {
                    if (all_will_be_matched) {
                        lexer->resultSymbol = PIPE_TABLE_LINE_ENDING;
                        return true;
                    }
                }
                else {
                    lexer->resultSymbol = SOFT_LINE_ENDING;
        ...
        if (valid_symbols[LINE_ENDING]) {
            ...
            // reset some state variables
            s->state &= (~STATE_WAS_SOFT_LINE_BREAK);
            lexer->resultSymbol = LINE_ENDING;
            return true;
        }
    }
    return false;
...
void* Create(void) {
    ...
    s->open_blocks.items = (Block*)scanner_calloc(1, sizeof(Block));
