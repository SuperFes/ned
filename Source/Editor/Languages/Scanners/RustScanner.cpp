// The rust external scanner, ported from https://github.com/tree-sitter/tree-sitter-rust (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::rust {
...
void* Create() {
    return scanner_calloc(1, sizeof(Scanner));
}
...
static inline bool is_num_char(int32_t c) {
...
static inline bool process_string(Lexer* lexer) {
...
static inline bool scan_raw_string_start(Scanner* scanner, Lexer* lexer) {
    ...
    lexer->resultSymbol = RAW_STRING_LITERAL_START;
    return true;
}

static inline bool scan_raw_string_content(Scanner* scanner, Lexer* lexer) {
    for (;;) {
        if (lexer->eof(lexer)) {
            return false;
        }
        if (lexer->lookahead == '"') {
            ...
            if (hash_count == scanner->opening_hash_count) {
                lexer->resultSymbol = RAW_STRING_LITERAL_CONTENT;
                return true;
            }
        }
        else {
...
static inline bool scan_raw_string_end(Scanner* scanner, Lexer* lexer) {
    advance(lexer);
    for (unsigned i = 0; i < scanner->opening_hash_count; i++) {
        advance(lexer);
    }
    lexer->resultSymbol = RAW_STRING_LITERAL_END;
    return true;
}

static inline bool process_float_literal(Lexer* lexer) {
...
static inline bool process_line_doc_content(Lexer* lexer) {
    lexer->resultSymbol = LINE_DOC_CONTENT;
    for (;;) {
        if (lexer->eof(lexer)) {
            return true;
        }
...
static inline void process_left_asterisk(BlockCommentProcessing* processing, char current, Lexer* lexer) {
    if (current == '*') {
        lexer->markEnd(lexer);
...
static inline bool process_block_comment(Lexer* lexer, const bool* valid_symbols) {
    ...
    if (valid_symbols[BLOCK_INNER_DOC_MARKER] && first == '!') {
        lexer->resultSymbol = BLOCK_INNER_DOC_MARKER;
        advance(lexer);
        return true;
    }
    if (valid_symbols[BLOCK_OUTER_DOC_MARKER] && first == '*') {
        ...
        if (lexer->lookahead != '*') {
            lexer->resultSymbol = BLOCK_OUTER_DOC_MARKER;
            return true;
        }
    }
    else {
...
static bool Scan(void* payload_, Lexer* lexer, const bool* valid_symbols) {
    ...
    */
    ...
    if (valid_symbols[STRING_CLOSE] && lexer->lookahead == '"') {
        advance(lexer);
        lexer->resultSymbol = STRING_CLOSE;
        lexer->markEnd(lexer);
        return true;
    }
...
NED_TREE_SITTER_SCANNER_EXPORTS(rust, ned::editor::languages::scanners::rust)
