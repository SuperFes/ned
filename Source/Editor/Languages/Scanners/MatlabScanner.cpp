// The matlab external scanner, ported from https://github.com/acristoffers/tree-sitter-matlab (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::matlab {
...
static inline bool consume_char(char chr, Lexer* lexer)
{
...
static inline void consume_identifier(Lexer* lexer, char* buffer)
{
    size_t size = 0;
...
static inline int skip_whitespaces(Lexer* lexer)
{
...
static inline int consume_whitespaces(Lexer* lexer)
{
    int skipped = 0;
...
static inline void consume_whitespaces_once(Lexer* lexer)
{
    while (!lexer->eof(lexer) && iswspace(lexer->lookahead)) {
...
static inline void consume_comment_line(Lexer* lexer)
{
    ...
    int skipped)
{
    ...
    if (entry_delimiter && !percent && !line_continuation) {
        if (iswdigit(lexer->lookahead)) {
            lexer->resultSymbol = ENTRY_DELIMITER;
            return true;
        }
        if (lexer->lookahead == '\'') {
            advance(lexer);
            lexer->resultSymbol = CTRANSPOSE;
            lexer->markEnd(lexer);
            return skipped == 0;
        }
        return false;
...
static bool scan_command(Scanner* scanner, Lexer* lexer, const bool* valid_symbols)
{
    ...
    if (lexer->lookahead == '!') {
        ...
        lexer->resultSymbol = COMMAND_NAME;
        lexer->markEnd(lexer);
        while (iswspace_matlab(lexer->lookahead)) {
            advance(lexer);
        }
...
skip_command_check:
    ...
    if (is_eol(lexer->lookahead)) {
        lexer->resultSymbol = valid_symbols[CATCH_IDENTIFIER] ? CATCH_IDENTIFIER : COMMAND_NAME;
        return true;
    }
    ...
    if (skipped & 2) {
        ...
        if (valid_symbols[CATCH_IDENTIFIER]) {
            lexer->resultSymbol = CATCH_IDENTIFIER;
            return true;
        }
    ...
    if (is_punct_char(lexer->lookahead)) {
        ...
        if (iswspace_matlab(second)) {
            ...
            if (is_invalid) {
                ...
                lexer->resultSymbol = scanner->is_inside_command ? COMMAND_NAME : IDENTIFIER;
                return true;
            }
...
static bool scan_command_argument(Scanner* scanner, Lexer* lexer)
{
    ...
    if (scanner->is_shell_scape) {
        ...
        lexer->resultSymbol = COMMAND_ARGUMENT;
        lexer->markEnd(lexer);
        while (iswspace_matlab(lexer->lookahead)) {
            advance(lexer);
        }
...
static bool scan_string_open(Scanner* scanner, Lexer* lexer)
{
    switch (lexer->lookahead) {
    case '"':
        scanner->string_delimiter = '"';
        advance(lexer);
        lexer->resultSymbol = DOUBLE_QUOTE_STRING_START;
        lexer->markEnd(lexer);
        return true;
    case '\'':
        scanner->string_delimiter = '\'';
        advance(lexer);
        lexer->resultSymbol = SINGLE_QUOTE_STRING_START;
        lexer->markEnd(lexer);
...
static bool scan_string_close(Scanner* scanner, Lexer* lexer)
{
    if (lexer->lookahead == scanner->string_delimiter) {
        ...
        lexer->resultSymbol = scanner->string_delimiter == '"' ? DOUBLE_QUOTE_STRING_END
                                                                : SINGLE_QUOTE_STRING_END;
        lexer->markEnd(lexer);
    ...
    if (lexer->lookahead == '%') {
        ...
        if (lexer->lookahead == '%') {
            advance(lexer);
            lexer->resultSymbol = FORMATTING_SEQUENCE;
            lexer->markEnd(lexer);
            return true;
        }
    ...
    if (lexer->lookahead == '\\') {
        ...
        if (lexer->lookahead == 'x') {
            ...
            while (!lexer->eof(lexer)) {
                ...
                if (!is_valid) {
                    lexer->resultSymbol = ESCAPE_SEQUENCE;
                    lexer->markEnd(lexer);
                    return true;
                }
...
static inline bool scan_multioutput_var_start(Lexer* lexer)
{
    advance(lexer);
    lexer->resultSymbol = MULTIOUTPUT_VARIABLE_START;
    lexer->markEnd(lexer);
...
static bool scan_entry_delimiter(Lexer* lexer, int skipped)
{
    lexer->markEnd(lexer);
    lexer->resultSymbol = ENTRY_DELIMITER;
...
static bool scan_transpose(Lexer* lexer)
{
    if (lexer->lookahead == '\'') {
        advance(lexer);
        lexer->markEnd(lexer);
        lexer->resultSymbol = TRANSPOSE;
        return true;
    }
