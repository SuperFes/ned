// The yaml external scanner, ported from https://github.com/tree-sitter-grammars/tree-sitter-yaml (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::yaml {
...
// --- schema.core.c (the core schema's tag scanner, from the same grammar) ---
#include <stdint.h>
...
#define RET_SYM(RESULT_SYMBOL)               \
    {                                        \
        flush(scanner);                      \
        lexer->resultSymbol = RESULT_SYMBOL; \
        return true;                         \
    }
...
static unsigned serialize(Scanner* scanner, char* buffer) {
    ...
    *(int16_t*)&buffer[size] = scanner->blk_imp_tab;
    ...
    for (; typ_itr != typ_end && size < kSerializationBufferSize; ++typ_itr, ++len_itr) {
...
static inline void adv(Scanner* scanner, Lexer* lexer) {
    scanner->cur_col++;
    scanner->cur_chr = lexer->lookahead;
    lexer->advance(lexer, false);
}

static inline void adv_nwl(Scanner* scanner, Lexer* lexer) {
    scanner->cur_row++;
    scanner->cur_col = 0;
    scanner->cur_chr = lexer->lookahead;
    lexer->advance(lexer, false);
...
static inline void skp(Scanner* scanner, Lexer* lexer) {
    scanner->cur_col++;
    scanner->cur_chr = lexer->lookahead;
    lexer->advance(lexer, true);
}

static inline void skp_nwl(Scanner* scanner, Lexer* lexer) {
    scanner->cur_row++;
    scanner->cur_col = 0;
    scanner->cur_chr = lexer->lookahead;
    lexer->advance(lexer, true);
...
static inline void mrk_end(Scanner* scanner, Lexer* lexer) {
...
static char scn_uri_esc(Scanner* scanner, Lexer* lexer) {
    if (lexer->lookahead != '%') {
        return SCN_STOP;
    }
...
static char scn_ns_uri_char(Scanner* scanner, Lexer* lexer) {
...
static char scn_ns_tag_char(Scanner* scanner, Lexer* lexer) {
...
static bool scn_dir_bgn(Scanner* scanner, Lexer* lexer) {
    adv(scanner, lexer);
...
static bool scn_dir_yml_ver(Scanner* scanner, Lexer* lexer, Symbol result_symbol) {
    uint16_t n1 = 0;
    uint16_t n2 = 0;
...
static bool scn_tag_hdl_tal(Scanner* scanner, Lexer* lexer) {
    if (lexer->lookahead == '!') {
        adv(scanner, lexer);
        return true;
    }
...
static bool scn_dir_tag_hdl(Scanner* scanner, Lexer* lexer, Symbol result_symbol) {
    if (lexer->lookahead == '!') {
        adv(scanner, lexer);
...
static bool scn_dir_tag_pfx(Scanner* scanner, Lexer* lexer, Symbol result_symbol) {
    if (lexer->lookahead == '!') {
        adv(scanner, lexer);
    }
...
static bool scn_dir_rsv_prm(Scanner* scanner, Lexer* lexer, Symbol result_symbol) {
    if (!is_ns_char(lexer->lookahead)) {
        return false;
    }
    adv(scanner, lexer);
...
static bool scn_tag(Scanner* scanner, Lexer* lexer, Symbol result_symbol) {
    if (lexer->lookahead != '!') {
        return false;
    }
    adv(scanner, lexer);
...
static bool scn_acr_bgn(Scanner* scanner, Lexer* lexer, Symbol result_symbol) {
    if (lexer->lookahead != '&') {
        return false;
    }
    adv(scanner, lexer);
...
static bool scn_acr_ctn(Scanner* scanner, Lexer* lexer, Symbol result_symbol) {
...
static bool scn_als_bgn(Scanner* scanner, Lexer* lexer, Symbol result_symbol) {
    if (lexer->lookahead != '*') {
        return false;
    }
    adv(scanner, lexer);
...
static bool scn_als_ctn(Scanner* scanner, Lexer* lexer, Symbol result_symbol) {
    while (is_ns_anchor_char(lexer->lookahead)) {
        adv(scanner, lexer);
    }
    mrk_end(scanner, lexer);
...
static bool scn_dqt_esc_seq(Scanner* scanner, Lexer* lexer, Symbol result_symbol) {
    uint16_t i;
...
static bool scn_drs_doc_end(Scanner* scanner, Lexer* lexer) {
...
static bool scn_dqt_str_cnt(Scanner* scanner, Lexer* lexer, Symbol result_symbol) {
...
static bool scn_sqt_str_cnt(Scanner* scanner, Lexer* lexer, Symbol result_symbol) {
...
static bool scn_blk_str_bgn(Scanner* scanner, Lexer* lexer, Symbol result_symbol) {
    if (lexer->lookahead != '|' && lexer->lookahead != '>') {
        return false;
    }
    adv(scanner, lexer);
...
static bool scn_blk_str_cnt(Scanner* scanner, Lexer* lexer, Symbol result_symbol) {
    if (!is_ns_char(lexer->lookahead)) {
        return false;
    }
    if (scanner->cur_col == 0 && scn_drs_doc_end(scanner, lexer)) {
...
static char scn_pln_cnt(Scanner* scanner, Lexer* lexer, bool (*is_plain_safe)(int32_t)) {
    ...
    if (maybe_sgl_pln_blk || maybe_sgl_pln_flw || maybe_mtl_pln_blk || maybe_mtl_pln_flw) {
        ...
        if (scanner->end_row == bgn_row) {
            if (maybe_sgl_pln_blk) {
                MAY_UPD_IMP_COL();
                RET_SYM(is_r ? SGL_PLN_SYM(R, BLK) : is_br ? SGL_PLN_SYM(BR, BLK)
                                                           : SGL_PLN_SYM(B, BLK));
            }
...
NED_TREE_SITTER_SCANNER_EXPORTS(yaml, ned::editor::languages::scanners::yaml)
