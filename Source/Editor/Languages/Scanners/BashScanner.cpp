// The bash external scanner, ported from https://github.com/tree-sitter/tree-sitter-bash (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::bash {
...
static unsigned serialize(Scanner* scanner, char* buffer) {
    ...
    for (uint32_t i = 0; i < scanner->heredocs.size; i++) {
        ...
        if (size + 3 + sizeof(uint32_t) + heredoc->delimiter.size >= kSerializationBufferSize) {
            return 0;
        }
 ...
 * IFS.
 */
static bool advance_word(Lexer* lexer, String* unquoted_word) {
    bool empty = true;
...
static inline bool scan_bare_dollar(Lexer* lexer) {
    ...
    if (lexer->lookahead == '$') {
        advance(lexer);
        lexer->resultSymbol = BARE_DOLLAR;
        lexer->markEnd(lexer);
...
static bool scan_heredoc_start(Heredoc* heredoc, Lexer* lexer) {
...
static bool scan_heredoc_end_identifier(Heredoc* heredoc, Lexer* lexer) {
...
static bool scan_heredoc_content(Scanner* scanner, Lexer* lexer, enum TokenType middle_type,
    ...
    for (;;) {
        switch (lexer->lookahead) {
            ...
            case '$': {
                ...
                if (did_advance) {
                    lexer->markEnd(lexer);
                    lexer->resultSymbol = middle_type;
                ...
                if (middle_type == HEREDOC_BODY_BEGINNING && lexer->getColumn(lexer) == 0) {
                    lexer->resultSymbol = middle_type;
                    heredoc->started    = true;
                    return true;
                }
            ...
            case '\n': {
                ...
                lexer->resultSymbol = heredoc->started ? middle_type : end_type;
                lexer->markEnd(lexer);
                if (scan_heredoc_end_identifier(heredoc, lexer)) {
                    if (lexer->resultSymbol == HEREDOC_END) {
...
static bool scan(Scanner* scanner, Lexer* lexer, const bool* valid_symbols) {
    ...
    if (valid_symbols[IMMEDIATE_DOUBLE_HASH] && !in_error_recovery(valid_symbols)) {
        ...
        if (lexer->lookahead == '#') {
            ...
            if (lexer->lookahead == '#') {
                advance(lexer);
                if (lexer->lookahead != '}') {
                    lexer->resultSymbol = IMMEDIATE_DOUBLE_HASH;
                    lexer->markEnd(lexer);
                    return true;
                }
            }
    ...
    if (valid_symbols[EXTERNAL_EXPANSION_SYM_HASH] && !in_error_recovery(valid_symbols)) {
        if (lexer->lookahead == '#' || lexer->lookahead == '=' || lexer->lookahead == '!') {
            lexer->resultSymbol = lexer->lookahead == '#'   ? EXTERNAL_EXPANSION_SYM_HASH
    ...
    if (valid_symbols[EMPTY_VALUE]) {
        if (iswspace(lexer->lookahead) || lexer->eof(lexer) || lexer->lookahead == ';' || lexer->lookahead == '&') {
            lexer->resultSymbol = EMPTY_VALUE;
            return true;
        }
    }
            ...
            if (iswspace(lexer->lookahead) && advanced_once) {
                ...
                if (lexer->lookahead == '}' && valid_symbols[CLOSING_BRACE]) {
                    if (valid_symbols[EXPANSION_WORD]) {
                        lexer->markEnd(lexer);
                        lexer->resultSymbol = EXPANSION_WORD;
                        return true;
                    }
                    return false;
                }
                lexer->resultSymbol = TEST_OPERATOR;
                return true;
            }
            if (iswspace(lexer->lookahead) && valid_symbols[EXTGLOB_PATTERN]) {
                lexer->resultSymbol = EXTGLOB_PATTERN;
                return true;
            }
        }
    ...
    if ((valid_symbols[VARIABLE_NAME] || valid_symbols[FILE_DESCRIPTOR] || valid_symbols[HEREDOC_ARROW]) &&
        ...
        for (;;) {
            if ((lexer->lookahead == ' ' || lexer->lookahead == '\t' || lexer->lookahead == '\r' ||
                ...
                if (lexer->eof(lexer)) {
                    lexer->markEnd(lexer);
                    lexer->resultSymbol = VARIABLE_NAME;
                    return true;
                }
        ...
        if (valid_symbols[HEREDOC_ARROW] && lexer->lookahead == '<') {
            ...
            if (lexer->lookahead == '<') {
                ...
                if (lexer->lookahead == '-') {
                    ...
                    lexer->resultSymbol = HEREDOC_ARROW_DASH;
                }
                    ...
                    Heredoc heredoc = heredoc_new();
                    array_push(&scanner->heredocs, heredoc);
                    lexer->resultSymbol = HEREDOC_ARROW;
                }
                return true;
            }
            return false;
        ...
        if (is_number && valid_symbols[FILE_DESCRIPTOR] && (lexer->lookahead == '>' || lexer->lookahead == '<')) {
            lexer->resultSymbol = FILE_DESCRIPTOR;
            return true;
        }
...
regex:
    if ((valid_symbols[REGEX] || valid_symbols[REGEX_NO_SLASH] || valid_symbols[REGEX_NO_SPACE]) &&
        ...
        if ((lexer->lookahead != '"' && lexer->lookahead != '\'') ||
            ...
            while (!state.done) {
                ...
                if (!state.done) {
                    if (valid_symbols[REGEX]) {
                        ...
                        if (lexer->lookahead == '/') {
                            lexer->markEnd(lexer);
                            lexer->resultSymbol = REGEX_NO_SLASH;
                        ...
                        if (lexer->lookahead == '\\') {
                            ...
                            if (iswspace(lexer->lookahead)) {
                                lexer->resultSymbol = REGEX_NO_SPACE;
                                lexer->markEnd(lexer);
                                return true;
                            }
                        }
            ...
            lexer->resultSymbol = valid_symbols[REGEX_NO_SLASH]   ? REGEX_NO_SLASH
...
brace_start:
    if (valid_symbols[BRACE_START] && !in_error_recovery(valid_symbols)) {
        ...
        lexer->resultSymbol = BRACE_START;
        return true;
    }
...
NED_TREE_SITTER_SCANNER_EXPORTS(bash, ned::editor::languages::scanners::bash)
