...
namespace ned::editor::languages::scanners::hcl {
...
#define STRING_RESIZE(vec, _cap)                                                         \
    {                                                                                    \
        VoidPtr tmp = scanner_realloc((vec).data, ((_cap) + 1) * sizeof((vec).data[0])); \
        assert(tmp.p != NULL);                                                           \
        (vec).data = tmp;                                                                \
...
static String string_new() {
    return (String){.cap = 16, .len = 0, .data = scanner_calloc(1, sizeof(char) * 17)};
}
...
static unsigned serialize(Scanner* scanner, char* buf) {
    ...
    for (uint32_t i = 0; i < scanner->context_stack.len; i++) {
        ...
        if (size + sizeof(ContextType) + sizeof(uint32_t) + context->heredoc_identifier.len >= kSerializationBufferSize) {
            return 0;
        }
...
static inline bool accept_inplace(Lexer* lexer, enum TokenType token) {
    lexer->resultSymbol = token;
    return true;
}

static inline bool accept_and_advance(Lexer* lexer, enum TokenType token) {
    advance(lexer);
    return accept_inplace(lexer, token);
}

static inline bool consume_wxdigit(Lexer* lexer) {
    advance(lexer);
    return iswxdigit(lexer->lookahead);
}

static inline bool skip_comment(Lexer* lexer) {
...
void* Create() {
    ...
    scanner->context_stack.data = scanner_calloc(1, sizeof(Context));
    return scanner;
}
...
NED_TREE_SITTER_SCANNER_EXPORTS(hcl, ned::editor::languages::scanners::hcl)
