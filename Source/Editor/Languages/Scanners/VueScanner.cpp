// The vue external scanner, ported from https://github.com/tree-sitter-grammars/tree-sitter-vue (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::vue {
...
static bool scan_start_tag_name(Scanner *scanner, Lexer *lexer) {
    ...
    switch (tag.type) {
        case TEMPLATE:
            lexer->resultSymbol = TEMPLATE_START_TAG_NAME;
            break;
        case SCRIPT:
...
static bool scan(Scanner *scanner, Lexer *lexer, const bool *valid_symbols) {
            ...
            switch (lexer->lookahead) {
                ...
                case '}':
                    if (valid_symbols[INTERPOLATION_TEXT]) {
                        ...
                        advance(lexer);
                        if (lexer->lookahead == '}') {
                            lexer->resultSymbol = INTERPOLATION_TEXT;
    ...
    loop_exit:
        if (advanced_once) {
            lexer->resultSymbol = TEXT_FRAGMENT;
            return true;
        }
    }
