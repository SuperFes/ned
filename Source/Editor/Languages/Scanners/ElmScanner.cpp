// The elm external scanner, ported from https://github.com/elm-tooling/tree-sitter-elm (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::elm {
...
static bool is_elm_space(Lexer *lexer) {
    return lexer->lookahead == ' ' || lexer->lookahead == '\r' ||
           lexer->lookahead == '\n';
}

static int checkForIn(Lexer *lexer, const bool *valid_symbols) {
...
static bool checkForSectionEndingToken(Lexer *lexer, const bool *valid_symbols) {
...
static void advance_to_line_end(Lexer *lexer) {
    while (true) {
        if (lexer->lookahead == '\n' || lexer->eof(lexer)) {
            break;
        }
...
static bool scan(Scanner *scanner, Lexer *lexer, const bool *valid_symbols) {
    ...
    if (scanner->runback.len > 0 && VEC_BACK(scanner->runback) == 0 &&
        ...
        lexer->resultSymbol = VIRTUAL_END_DECL;
        return true;
    }
    if (scanner->runback.len > 0 && VEC_BACK(scanner->runback) == 1 &&
        valid_symbols[VIRTUAL_END_SECTION]) {
        VEC_POP(scanner->runback);
        lexer->resultSymbol = VIRTUAL_END_SECTION;
        return true;
    }
    ...
    if (valid_symbols[STRING_CONTENT_MULTILINE]) {
        lexer->resultSymbol = STRING_CONTENT_MULTILINE;
        ...
        if (lexer->lookahead == ' ' || lexer->lookahead == '\r') {
            ...
            while (true) {
                if (lexer->lookahead == ' ') {
                    ...
                    scanner->indent_length = lexer->getColumn(lexer);
                    break;
                }
            }
            ...
            if (valid_symbols[MINUS_WITHOUT_TRAILING_WHITESPACE] &&
                ...
                if (can_call_mark_end) {
                    lexer->resultSymbol = MINUS_WITHOUT_TRAILING_WHITESPACE;
                    lexer->markEnd(lexer);
                    return true;
                }
                return false;
            ...
            // not call `lexer->markEnd(lexer)` after this point, or
    ...
    if (valid_symbols[VIRTUAL_OPEN_SECTION] && !lexer->eof(lexer)) {
        ...
        VEC_PUSH(scanner->indents, lexer->getColumn(lexer));
        lexer->resultSymbol = VIRTUAL_OPEN_SECTION;
        return true;
    }
    ...
    if (valid_symbols[GLSL_CONTENT]) {
        ...
        lexer->resultSymbol = GLSL_CONTENT;
        while (true) {
            switch (lexer->lookahead) {
                case '|':
                    lexer->markEnd(lexer);
