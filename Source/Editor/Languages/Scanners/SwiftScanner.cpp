// The swift external scanner, ported from https://github.com/alex-pinkus/tree-sitter-swift (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::swift {
...
void *Create() {
    return scanner_calloc(1, sizeof(struct ScannerState));
}
...
static enum TokenType find_possible_compiler_directive(Lexer *lexer) {
...
static bool eat_raw_str_part(
    ...
    while (lexer->lookahead != '\0') {
        uint8_t last_char = '\0';
        lexer->markEnd(lexer); // We always want to parse thru the start of the string so far
...
static bool Scan(
    ...
    if (comment == STOP_PARSING_TOKEN_FOUND) {
        lexer->markEnd(lexer);
        lexer->resultSymbol = comment_result;
        return true;
    }
    ...
    if (saw_operator && (!has_ws_result || is_cross_semi_token(operator_result))) {
        lexer->resultSymbol = operator_result;
        if (has_ws_result) lexer->markEnd(lexer);
        return true;
    }
    ...
    if (saw_raw_str_part) {
        lexer->resultSymbol = raw_str_result;
        return true;
    }
