// The fsharp external scanner, ported from https://github.com/ionide/tree-sitter-fsharp (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::fsharp {
...
static inline bool scan_block_comment(Lexer *lexer) {
  lexer->markEnd(lexer);
  if (lexer->lookahead != '(')
    return false;
...
static inline bool is_infix_op_start(Lexer *lexer) {
  switch (lexer->lookahead) {
  case '+':
    skip(lexer);
...
static inline bool is_bracket_end(Lexer *lexer) {
  switch (lexer->lookahead) {
  case ')':
  case ']':
  case '}':
...
static bool scan(Scanner *scanner, Lexer *lexer, const bool *valid_symbols) {
  if (valid_symbols[ERROR_SENTINEL]) {
    ...
    if (scanner->preprocessor_indents.size > 0) {
      ...
      lexer->resultSymbol = PREPROC_END;
      return true;
    }
  ...
  if (valid_symbols[TRIPLE_QUOTE_CONTENT]) {
    ...
    lexer->resultSymbol = TRIPLE_QUOTE_CONTENT;
    return true;
  }
  ...
  uint32_t indent_length = lexer->getColumn(lexer);
    ...
    if (lexer->lookahead == '\n') {
      ...
      if (lexer->lookahead == 'e') {
          ...
          if (lexer->lookahead == 's') {
            ...
            if (lexer->lookahead == 'e') {
              ...
              if (valid_symbols[PREPROC_ELSE]) {
                lexer->markEnd(lexer);
                lexer->resultSymbol = PREPROC_ELSE;
                return true;
              }
            }
          }
        ...
        if (found_end_of_line && valid_symbols[NEWLINE_NO_ALIGNED]) {
          lexer->resultSymbol = NEWLINE_NO_ALIGNED;
          return true;
        }
        return false;
      }
  ...
  if (valid_symbols[CLASS] && lexer->lookahead == 'c') {
    lexer->markEnd(lexer);
    indent_length = lexer->getColumn(lexer);
    advance(lexer);
  ...
  if (lexer->lookahead == 't' &&
    ...
    advance(lexer);
    int16_t token_indent_level = lexer->getColumn(lexer);
    if (lexer->lookahead == 'l') {
      advance(lexer);
      if (lexer->lookahead == 's' &&
