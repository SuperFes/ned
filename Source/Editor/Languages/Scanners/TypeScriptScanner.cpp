// The typescript external scanner, ported from its upstream repository (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::typescript {
...
static bool scan_template_chars(Lexer* lexer) {
...
static bool scan_whitespace_and_comments(Lexer* lexer, bool* scanned_comment) {
    for (;;) {
...
static bool scan_automatic_semicolon(Lexer* lexer, const bool* valid_symbols, bool* scanned_comment) {
...
static bool scan_ternary_qmark(Lexer* lexer) {
    for (;;) {
        if (!iswspace(lexer->lookahead)) {
            break;
        }
...
static bool scan_closing_comment(Lexer* lexer) {
...
static bool scan_jsx_text(Lexer* lexer) {
...
NED_TREE_SITTER_SCANNER_EXPORTS(typescript, ned::editor::languages::scanners::typescript)
