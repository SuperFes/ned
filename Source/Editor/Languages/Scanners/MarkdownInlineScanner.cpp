...
static bool parse_star(Scanner* s, Lexer* lexer, const bool* valid_symbols) {
    ...
    if (s->num_emphasis_delimiters_left > 0) {
        ...
        if ((s->state & STATE_EMPHASIS_DELIMITER_IS_OPEN) &&
            ...
            lexer->resultSymbol = EMPHASIS_OPEN_STAR;
            s->num_emphasis_delimiters_left--;
            return true;
        }
        if (valid_symbols[EMPHASIS_CLOSE_STAR]) {
            lexer->resultSymbol = EMPHASIS_CLOSE_STAR;
            s->num_emphasis_delimiters_left--;
            return true;
        }
    }
...
static bool parse_underscore(Scanner* s, Lexer* lexer,
                             const bool* valid_symbols) {
    lexer->advance(lexer, false);
    // If `num_emphasis_delimiters_left` is not zero then we already decided
    // that this should be part of an emphasis delimiter run, so interpret it as
    ...
    if (s->num_emphasis_delimiters_left > 0) {
        ...
        if ((s->state & STATE_EMPHASIS_DELIMITER_IS_OPEN) &&
            ...
            s->state &= (~STATE_EMPHASIS_DELIMITER_IS_OPEN);
            lexer->resultSymbol = EMPHASIS_OPEN_UNDERSCORE;
            s->num_emphasis_delimiters_left--;
            return true;
        }
        if (valid_symbols[EMPHASIS_CLOSE_UNDERSCORE]) {
            lexer->resultSymbol = EMPHASIS_CLOSE_UNDERSCORE;
            s->num_emphasis_delimiters_left--;
            return true;
        }
    }
...
NED_TREE_SITTER_SCANNER_EXPORTS(markdown_inline, ned::editor::languages::scanners::markdown_inline)
