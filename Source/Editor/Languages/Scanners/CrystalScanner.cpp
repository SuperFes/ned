// The crystal external scanner, ported from https://github.com/crystal-lang-tools/tree-sitter-crystal (src/scanner.c, MIT
...
static void lex_skip(State *state, Lexer *lexer) {
...
// symbol in glibc
static void lex_advance(Lexer *lexer) {
    lexer->advance(lexer, false);
}

static bool next_char_is_identifier(Lexer *lexer) {
...
static bool check_for_heredoc_start(State *state, Lexer *lexer, const bool *valid_symbols) {
    ...
    if (valid_symbols[HEREDOC_BODY_START]
        ...
        lexer->resultSymbol = HEREDOC_BODY_START;
        return true;
    }
    return false;
}

static bool scan_whitespace(State *state, Lexer *lexer, const bool *valid_symbols) {
                ...
                if (crossed_newline) {
                    if (lexer->lookahead == '.') {
                        ...
                        if (iswspace(lexer->lookahead)) {
                            lex_advance(lexer);
                            lexer->markEnd(lexer);
                            lexer->resultSymbol = TYPE_FIELD_COLON;
                        } else {
                            lexer->resultSymbol = LINE_BREAK;
                        }
                    } else {
...
static ScanResult scan_string_contents(State *state, Lexer *lexer, const bool *valid_symbols) {
    bool found_content = false;
    LiteralType active_type;

    // This may be overridden later.
    lexer->resultSymbol = DELIMITED_STRING_CONTENTS;
            ...
            case '\f':
                if (active_type == STRING_ARRAY || active_type == SYMBOL_ARRAY) {
                    ...
                    if (found_content) {
                        ...
                        lexer->resultSymbol = DELIMITED_ARRAY_ELEMENT_END;
                        return SR_STOP;
                    }
                }
                break;
...
static bool scan_heredoc_contents(State *state, Lexer *lexer, const bool *valid_symbols) {
    ...
    start_of_line:
        ...
        if (valid_symbols[HEREDOC_END] && !lexer->eof(lexer) && lexer->getColumn(lexer) == 0) {
            if (found_content) {
                lexer->markEnd(lexer);
        ...
        lexer->resultSymbol = HEREDOC_CONTENT;
...
static bool match_macro_keyword(Lexer *lexer, const char keyword[]) {
...
static ScanResult scan_macro_contents(State *state, Lexer *lexer, const bool *valid_symbols) {
                    ...
                    if (lexer->lookahead == 'e') {
                        ...
                        if (lexer->lookahead == 'n') {
                            if (match_macro_keyword(lexer, "nd")) {
                                lexer->resultSymbol = MACRO_DELIMITER_END;
                                return SR_STOP;
                            }
                            ...
                            if (lexer->lookahead == 's') {
                                ...
                                if (lexer->lookahead == 'e') {
                                    if (match_macro_keyword(lexer, "e")) {
                                        lexer->resultSymbol = MACRO_DELIMITER_ELSE;
                                        return SR_STOP;
                                    }
                                } else if (lexer->lookahead == 'i') {
                                    if (match_macro_keyword(lexer, "if")) {
                                        lexer->resultSymbol = MACRO_DELIMITER_ELSIF;
                                        return SR_STOP;
                                    }
                                }
                            }
...
static ScanResult scan_symbol_content(Lexer *lexer) {
    int32_t lookahead = lexer->lookahead;
    ...
    if (('A' <= lookahead && lookahead <= 'Z')
        ...
        lexer->resultSymbol = UNQUOTED_SYMBOL_CONTENT;
        lex_advance(lexer);
...
static bool scan_regex_modifier(State *state, Lexer *lexer) {
    if (!state->has_leading_whitespace) {
        ...
        for (;;) {
            ...
            if (found_modifier) {
                lexer->resultSymbol = REGEX_MODIFIER;
                return true;
            } else {
                break;
            }
...
static void advance_space(Lexer *lexer) {
    while (lexer->lookahead == ' ' || lexer->lookahead == '\t') {
        lex_advance(lexer);
    }
}
...
static void advance_space_and_newline(Lexer *lexer) {
...
static void consume_const(Lexer *lexer) {
...
static void consume_string_literal(Lexer *lexer) {
...
static LookaheadResult lookahead_delimiter_or_type_suffix(State *state, Lexer *lexer) {
    if (lexer->eof(lexer)) { return static_cast<LookaheadResult>(true); }

    switch (lexer->lookahead) {
        case '.':
            lex_advance(lexer);
            advance_space_and_newline(lexer);
            if (lexer->lookahead != 'c') { return static_cast<LookaheadResult>(false); }
            lex_advance(lexer);
            if (lexer->lookahead != 'l') { return static_cast<LookaheadResult>(false); }
            lex_advance(lexer);
            if (lexer->lookahead != 'a') { return static_cast<LookaheadResult>(false); }
            lex_advance(lexer);
            if (lexer->lookahead != 's') { return static_cast<LookaheadResult>(false); }
            lex_advance(lexer);
            if (lexer->lookahead != 's') { return static_cast<LookaheadResult>(false); }
            lex_advance(lexer);
...
static LookaheadResult lookahead_start_of_named_tuple_entry(Lexer *lexer, bool started) {
    if (started
...
static LookaheadResult lookahead_start_of_type(State *state, Lexer *lexer) {
...
static bool inner_scan(void *payload, Lexer *lexer, const bool *valid_symbols) {
    ...
    if (valid_symbols[MACRO_START] && !valid_symbols[ERROR_RECOVERY]) {
        ...
        lexer->resultSymbol = MACRO_START;
        return true;
    }
    ...
    lexer->resultSymbol = NONE;

    if (!scan_whitespace(state, lexer, valid_symbols)) {
        return false;
    }

    if (lexer->resultSymbol != NONE) {
        return true;
    }
    ...
    if (valid_symbols[PERCENT_LITERAL_END] && HAS_ACTIVE_LITERAL(state)) {
        if (lexer->lookahead == ACTIVE_LITERAL(state)->closing_char) {
            ...
            lexer->resultSymbol = PERCENT_LITERAL_END;
            return true;
        }
    }
    ...
    if (valid_symbols[STRING_LITERAL_END] && HAS_ACTIVE_LITERAL(state)) {
        if (lexer->lookahead == ACTIVE_LITERAL(state)->closing_char) {
            lex_advance(lexer);
            (void)POP_LITERAL(state);
            lexer->resultSymbol = STRING_LITERAL_END;
            return true;
        }
    }
    ...
    if (valid_symbols[COMMAND_LITERAL_END] && HAS_ACTIVE_LITERAL(state)) {
        if (lexer->lookahead == ACTIVE_LITERAL(state)->closing_char) {
            lex_advance(lexer);
            (void)POP_LITERAL(state);
            lexer->resultSymbol = COMMAND_LITERAL_END;
            return true;
        }
    }

    if (valid_symbols[DELIMITED_ARRAY_ELEMENT_START] && HAS_ACTIVE_LITERAL(state)) {
        lexer->resultSymbol = DELIMITED_ARRAY_ELEMENT_START;
        return true;
    }
...
#define BRACE_TYPE  (valid_symbols[START_OF_TUPLE_TYPE] || valid_symbols[START_OF_NAMED_TUPLE_TYPE])
            ...
            if (BRACE_BLOCK || BRACE_EXPR || BRACE_TYPE) {
                if (BRACE_BLOCK && BRACE_EXPR && BRACE_TYPE) {
                    ...
                    if (valid_symbols[START_OF_PARENLESS_ARGS]) {
                        lexer->resultSymbol = START_OF_BRACE_BLOCK;
                        return true;
                    }
                    ...
                    if (valid_symbols[END_OF_RANGE]) {
                        ...
                        switch (lookahead_start_of_named_tuple_entry(lexer, false)) {
                            case LOOKAHEAD_NAMED_TUPLE:
                                lexer->resultSymbol = START_OF_NAMED_TUPLE;
                                return true;

                            default:
                                lexer->resultSymbol = START_OF_HASH_OR_TUPLE;
                                return true;
                        }
                    }
                    ...
                    switch (lookahead_start_of_type(state, lexer)) {
                        case LOOKAHEAD_TYPE:
                            ...
                            lexer->resultSymbol = START_OF_TUPLE_TYPE;
                            return true;
                    ...
                    switch (lookahead_start_of_named_tuple_entry(lexer, false)) {
                        case LOOKAHEAD_NAMED_TUPLE:
                            ...
                            lexer->resultSymbol = START_OF_NAMED_TUPLE_TYPE;
                            return true;
        ...
        case '[':
            if (valid_symbols[START_OF_INDEX_OPERATOR]) {
                ...
                if (state->has_leading_whitespace && valid_symbols[START_OF_PARENLESS_ARGS]) {
                    ...
                    lex_advance(lexer);
                    lexer->resultSymbol = START_OF_INDEX_OPERATOR;
                    return true;
                }
            }
            break;
        ...
        case '+':
            if (valid_symbols[UNARY_PLUS] || valid_symbols[BINARY_PLUS]) {
                ...
                if (valid_symbols[UNARY_PLUS] && unary_priority) {
                    lexer->resultSymbol = UNARY_PLUS;
                } else if (valid_symbols[BINARY_PLUS]) {
                    lexer->resultSymbol = BINARY_PLUS;
                } else {
                    lexer->resultSymbol = UNARY_PLUS;
                }
        ...
        case '-':
            if (valid_symbols[UNARY_MINUS] || valid_symbols[BINARY_MINUS]) {
                ...
                if (valid_symbols[UNARY_MINUS] && unary_priority) {
                    lexer->resultSymbol = UNARY_MINUS;
        ...
        case '*':
            if (valid_symbols[POINTER_STAR] || valid_symbols[UNARY_STAR] || valid_symbols[BINARY_STAR] || valid_symbols[UNARY_DOUBLE_STAR] || valid_symbols[BINARY_DOUBLE_STAR]) {
                ...
                if (valid_symbols[POINTER_STAR] && !valid_symbols[ERROR_RECOVERY]) {
                    lexer->resultSymbol = POINTER_STAR;
                    return true;
                }
                ...
                if (lexer->lookahead == '*') {
                    ...
                    if (valid_symbols[UNARY_DOUBLE_STAR] && unary_priority) {
                        lexer->resultSymbol = UNARY_DOUBLE_STAR;
                        return true;
                    } else if (valid_symbols[BINARY_DOUBLE_STAR]) {
                        lexer->resultSymbol = BINARY_DOUBLE_STAR;
                        return true;
                ...
                if (valid_symbols[UNARY_STAR] && unary_priority) {
                    lexer->resultSymbol = UNARY_STAR;
                    return true;
                } else if (valid_symbols[BINARY_STAR]) {
                    lexer->resultSymbol = BINARY_STAR;
                    return true;
            ...
            if (
                ...
                if (lexer->lookahead == '+') {
                    ...
                    if (valid_symbols[BINARY_WRAPPING_PLUS]) {
                        lexer->markEnd(lexer);
                        lexer->resultSymbol = BINARY_WRAPPING_PLUS;
                        return true;
                    } else if (valid_symbols[UNARY_WRAPPING_PLUS]) {
                        lexer->markEnd(lexer);
                        lexer->resultSymbol = UNARY_WRAPPING_PLUS;
                        return true;
                    }
                ...
                if (lexer->lookahead == '-') {
                    ...
                    if (lexer->lookahead == '>') {
                        ...
                        if (unary_priority && valid_symbols[BLOCK_AMPERSAND]) {
                            lexer->resultSymbol = BLOCK_AMPERSAND;
                            return true;
                        } else if (valid_symbols[BINARY_AMPERSAND]) {
                            lexer->resultSymbol = BINARY_AMPERSAND;
                            return true;
                    ...
                    if (valid_symbols[BINARY_WRAPPING_MINUS]) {
                        lexer->markEnd(lexer);
                        lexer->resultSymbol = BINARY_WRAPPING_MINUS;
                        return true;
                    } else if (valid_symbols[UNARY_WRAPPING_MINUS]) {
                        lexer->markEnd(lexer);
                        lexer->resultSymbol = UNARY_WRAPPING_MINUS;
                        return true;
                    }
        ...
        case '/':
            if (valid_symbols[REGEX_START]
                ...
                if (lexer->lookahead == '=') {
                    if (valid_symbols[REGEX_START] || valid_symbols[BINARY_SLASH]) {
                        if (valid_symbols[REGEX_START] && !valid_symbols[BINARY_SLASH]) {
                            ...
                            lexer->resultSymbol = REGEX_START;
                            return true;
                ...
                if (lexer->lookahead == '/' && valid_symbols[BINARY_DOUBLE_SLASH]) {
                    ...
                    lexer->resultSymbol = BINARY_DOUBLE_SLASH;
                    return true;
                }

                if (valid_symbols[BINARY_SLASH] && !valid_symbols[REGEX_START]) {
                    lexer->resultSymbol = BINARY_SLASH;
                    return true;
        ...
        case '%':
            ...
            if (valid_symbols[STRING_PERCENT_LITERAL_START]
                ...
                switch (lexer->lookahead) {
                    ...
                    default:
                        if (valid_symbols[MODULO_OPERATOR]) {
                            lexer->resultSymbol = MODULO_OPERATOR;
                            return true;
                        }
                }
                ...
                if (opening_char) {
                    ...
                    lexer->resultSymbol = return_symbol;
        ...
        case '\\':
            if (valid_symbols[LINE_CONTINUATION]) {
                ...
                if (lexer->lookahead == '\n') {
                    lex_advance(lexer);
                    lexer->resultSymbol = LINE_CONTINUATION;
            ...
            if (valid_symbols[START_OF_SYMBOL] || valid_symbols[TYPE_FIELD_COLON]) {
                ...
                switch (lookahead) {
                    ...
                    case '~':
                        ...
                        lexer->resultSymbol = START_OF_SYMBOL;
                        return true;
                }
        ...
        case '.':
            if (valid_symbols[BEGINLESS_RANGE_OPERATOR] && !valid_symbols[START_OF_PARENLESS_ARGS]) {
                ...
                lexer->resultSymbol = BEGINLESS_RANGE_OPERATOR;
                return true;
            }
            break;
        case 'e':
            if (valid_symbols[REGULAR_ENSURE_KEYWORD] || valid_symbols[MODIFIER_ENSURE_KEYWORD]) {
                ...
                if (valid_symbols[MODIFIER_ENSURE_KEYWORD] && !valid_symbols[REGULAR_ENSURE_KEYWORD]) {
                    lexer->resultSymbol = MODIFIER_ENSURE_KEYWORD;
                    return true;
                } else if (valid_symbols[REGULAR_ENSURE_KEYWORD] && !valid_symbols[MODIFIER_ENSURE_KEYWORD]) {
                    lexer->resultSymbol = REGULAR_ENSURE_KEYWORD;
                    return true;
                } else {
                    // Both are valid
        ...
        case 'i':
            if (valid_symbols[REGULAR_IF_KEYWORD] || valid_symbols[MODIFIER_IF_KEYWORD]) {
                ...
                if (valid_symbols[MODIFIER_IF_KEYWORD] && !valid_symbols[REGULAR_IF_KEYWORD]) {
                    lexer->resultSymbol = MODIFIER_IF_KEYWORD;
                    return true;
                } else if (valid_symbols[REGULAR_IF_KEYWORD] && !valid_symbols[MODIFIER_IF_KEYWORD]) {
                    lexer->resultSymbol = REGULAR_IF_KEYWORD;
                    return true;
                } else {
                    // Both are valid
        ...
        case 'r':
            if (valid_symbols[REGULAR_RESCUE_KEYWORD] || valid_symbols[MODIFIER_RESCUE_KEYWORD]) {
                ...
                if (valid_symbols[MODIFIER_RESCUE_KEYWORD] && !valid_symbols[REGULAR_RESCUE_KEYWORD]) {
                    lexer->resultSymbol = MODIFIER_RESCUE_KEYWORD;
                    return true;
                } else if (valid_symbols[REGULAR_RESCUE_KEYWORD] && !valid_symbols[MODIFIER_RESCUE_KEYWORD]) {
                    lexer->resultSymbol = REGULAR_RESCUE_KEYWORD;
                    return true;
                } else {
                    // Both are valid
        ...
        case 'u':
            if (valid_symbols[REGULAR_UNLESS_KEYWORD] || valid_symbols[MODIFIER_UNLESS_KEYWORD]) {
                ...
                if (valid_symbols[MODIFIER_UNLESS_KEYWORD] && !valid_symbols[REGULAR_UNLESS_KEYWORD]) {
                    lexer->resultSymbol = MODIFIER_UNLESS_KEYWORD;
                    return true;
                } else if (valid_symbols[REGULAR_UNLESS_KEYWORD] && !valid_symbols[MODIFIER_UNLESS_KEYWORD]) {
                    lexer->resultSymbol = REGULAR_UNLESS_KEYWORD;
                    return true;
                } else {
                    // Both are valid
        ...
        case 'y':
            if (valid_symbols[END_OF_WITH_EXPRESSSION]) {
                ...
                lexer->resultSymbol = END_OF_WITH_EXPRESSSION;
                return true;
            }
            break;
...
void *Create(void) {
    ...
    state = scanner_calloc(1, sizeof(State));
...
static unsigned Serialize(void *payload_, char *buffer) {
    ...
    assert(offset <= kSerializationBufferSize);
