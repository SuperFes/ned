// The ruby external scanner, ported from https://github.com/tree-sitter/tree-sitter-ruby (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::ruby {
...
static inline void skip(Scanner* scanner, Lexer* lexer) {
...
static inline unsigned serialize(Scanner* scanner, char* buffer) {
    ...
    if (scanner->literal_stack.size * 5 + 2 >= kSerializationBufferSize) {
        return 0;
    }
    ...
    for (uint32_t i = 0; i < scanner->open_heredocs.size; i++) {
        ...
        if (size + 2 + heredoc->word.size >= kSerializationBufferSize) {
            return 0;
        }
...
static inline bool scan_whitespace(Scanner* scanner, Lexer* lexer, const bool* valid_symbols) {
    ...
    for (;;) {
        if (!valid_symbols[NO_LINE_BREAK] && valid_symbols[LINE_BREAK] && lexer->isAtIncludedRangeStart(lexer)) {
            lexer->markEnd(lexer);
...
static inline bool scan_operator(Lexer* lexer) {
    switch (lexer->lookahead) {
        // <, <=, <<, <=>
        case '<':
            advance(lexer);
...
static inline bool scan_symbol_identifier(Lexer* lexer) {
    if (lexer->lookahead == '@') {
        advance(lexer);
        if (lexer->lookahead == '@') {
            advance(lexer);
...
static inline bool scan_open_delimiter(Scanner* scanner, Lexer* lexer, Literal* literal, const bool* valid_symbols) {
    switch (lexer->lookahead) {
        case '"':
...
static inline void scan_heredoc_word(Lexer* lexer, Heredoc* heredoc) {
...
static inline bool scan_short_interpolation(Lexer* lexer, const bool has_content, const Symbol content_symbol) {
    ...
    if (start == '@' || start == '$') {
        ...
        if (is_short_interpolation) {
            lexer->resultSymbol = SHORT_INTERPOLATION;
            return true;
        }
    }
    return false;
...
static inline bool scan_heredoc_content(Scanner* scanner, Lexer* lexer) {
        ...
        if (position_in_word == heredoc->word.size) {
            ...
            if (lexer->lookahead == '\n' || lexer->lookahead == '\r') {
                if (has_content) {
                    ...
                    lexer->resultSymbol = HEREDOC_BODY_END;
                }
                return true;
            }
            has_content      = true;
...
static inline bool scan_literal_content(Scanner* scanner, Lexer* lexer) {
...
static inline bool scan(Scanner* scanner, Lexer* lexer, const bool* valid_symbols) {
    scanner->has_leading_whitespace = false;
    ...
    switch (lexer->lookahead) {
        ...
        case '<':
            if (valid_symbols[SINGLETON_CLASS_LEFT_ANGLE_LEFT_ANGLE]) {
                ...
                if (lexer->lookahead == '<') {
                    advance(lexer);
                    lexer->resultSymbol = SINGLETON_CLASS_LEFT_ANGLE_LEFT_ANGLE;
                    return true;
                }
                return false;
            }
                ...
                if (lexer->lookahead == '*') {
                    if (valid_symbols[HASH_SPLAT_STAR_STAR] || valid_symbols[BINARY_STAR_STAR]) {
                        ...
                        if (valid_symbols[BINARY_STAR_STAR] && !scanner->has_leading_whitespace) {
                            lexer->resultSymbol = BINARY_STAR_STAR;
                            return true;
                        }
                        if (valid_symbols[HASH_SPLAT_STAR_STAR] && !iswspace(lexer->lookahead)) {
                            lexer->resultSymbol = HASH_SPLAT_STAR_STAR;
                            return true;
                        }
            ...
            if (valid_symbols[UNARY_MINUS] || valid_symbols[UNARY_MINUS_NUM] || valid_symbols[BINARY_MINUS]) {
                ...
                if (lexer->lookahead != '=' && lexer->lookahead != '>') {
                    if (valid_symbols[UNARY_MINUS_NUM] &&
                        ...
                        lexer->resultSymbol = UNARY_MINUS_NUM;
                        return true;
                    }
            ...
            if (valid_symbols[SYMBOL_START]) {
                ...
                switch (lexer->lookahead) {
                    case '"':
                        ...
                        literal.allows_interpolation = true;
                        array_push(&scanner->literal_stack, literal);
                        lexer->resultSymbol = SYMBOL_START;
                        return true;
                    ...
                    default:
                        if (scan_symbol_identifier(lexer)) {
                            lexer->resultSymbol = SIMPLE_SYMBOL;
                            return true;
                        }
                }
        ...
        case '[':
            ...
            if (valid_symbols[ELEMENT_REFERENCE_BRACKET] &&
                ...
                advance(lexer);
                lexer->resultSymbol = ELEMENT_REFERENCE_BRACKET;
                return true;
            }
            break;
    ...
    if (((valid_symbols[HASH_KEY_SYMBOL] || valid_symbols[IDENTIFIER_SUFFIX]) &&
        ...
        if (valid_symbols[HASH_KEY_SYMBOL] && lexer->lookahead == ':') {
            ...
            advance(lexer);
            if (lexer->lookahead != ':') {
                lexer->resultSymbol = HASH_KEY_SYMBOL;
                return true;
            }
        }
            ...
            advance(lexer);
            if (lexer->lookahead != '=') {
                lexer->resultSymbol = validIdentifierSymbol;
                return true;
            }
        }
    ...
    if (valid_symbols[STRING_START]) {
        ...
        if (scan_open_delimiter(scanner, lexer, &literal, valid_symbols)) {
            array_push(&scanner->literal_stack, literal);
            lexer->resultSymbol = literal.type;
            return true;
        }
        return false;
    }
...
NED_TREE_SITTER_SCANNER_EXPORTS(ruby, ned::editor::languages::scanners::ruby)
