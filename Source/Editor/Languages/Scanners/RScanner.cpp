// The r external scanner, ported from https://github.com/r-lib/tree-sitter-r (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::r {
...
// of them inside `kSerializationBufferSize`. We expect
...
// `kSerializationBufferSize` after removing all other serialized
...
#define MAX_SCOPES_COUNT                                                                          \
    ((kSerializationBufferSize - RAW_STRING_CLOSING_BRACKET_SIZE - RAW_STRING_HYPHEN_COUNT_SIZE - \
...
static bool scopes_init(Scopes* scopes) {
    Scope* data = scanner_malloc(MAX_SCOPES_COUNT * SCOPE_SIZE);
    if (data == NULL) {
...
static Payload* payload_new(void) {
    Payload* payload = scanner_malloc(PAYLOAD_SIZE);
    if (payload == NULL) {
...
static inline void consume_whitespace_and_ignored_newlines(Lexer* lexer, Scopes* scopes) {
...
static inline bool scan_else(Lexer* lexer) {
    ...
    lexer->markEnd(lexer);
    lexer->resultSymbol = ELSE;

    return true;
}

static inline bool scan_else_with_leading_newlines(Lexer* lexer) {
    ...
    // `valid_symbols`. Returning `false` seems to make `lexer->resultSymbol = NEWLINE`
    // completely ignored.
    //
    // {
    //   if (cond) {
...
static inline bool scan_raw_string_open(Lexer* lexer, RawStringState* raw_string_state) {
    ...
    // Success!
    lexer->markEnd(lexer);
    lexer->resultSymbol = RAW_STRING_OPEN;
...
static inline bool scan_raw_string_content_or_close(Lexer* lexer, RawStringState* raw_string_state) {
    ...
    while (!lexer->eof(lexer)) {
        ...
        if (any_content) {
            ...
            // quoted strings.
            lexer->markEnd(lexer);
            lexer->resultSymbol = RAW_STRING_CLOSE;
        }
...
static inline bool scan_raw_string_close(Lexer* lexer, RawStringState* raw_string_state) {
...
static inline bool scan_semicolon(Lexer* lexer) {
    lexer->advance(lexer, false);
    lexer->markEnd(lexer);
...
static inline bool scan_newline(Lexer* lexer) {
    lexer->advance(lexer, false);
    lexer->markEnd(lexer);
    lexer->resultSymbol = NEWLINE;
    return true;
...
static inline bool scan_open_block(Lexer* lexer, Scopes* scopes, Scope scope, Symbol symbol) {
...
static inline bool scan_close_block(Lexer* lexer, Scopes* scopes, Scope scope, Symbol symbol) {
...
static inline bool scan_open_bracket_or_bracket2(Lexer* lexer, Scopes* scopes, const bool* valid_symbols) {
    ...
    if (valid_symbols[OPEN_BRACKET2] && lexer->lookahead == '[') {
        ...
        lexer->advance(lexer, false);
        lexer->markEnd(lexer);
        lexer->resultSymbol = OPEN_BRACKET2;
        return true;
    }
    ...
    if (valid_symbols[OPEN_BRACKET]) {
        ...
        lexer->markEnd(lexer);
        lexer->resultSymbol = OPEN_BRACKET;
        return true;
    }
...
static inline bool scan_close_bracket2(Lexer* lexer, Scopes* scopes) {
...
static bool scan(Lexer* lexer, Scopes* scopes, RawStringState* raw_string_state, const bool* valid_symbols) {
...
// `kSerializationBufferSize`, so we can't write more than this
// into the buffer!
static unsigned Serialize(void* payload_, char* buffer) {
    ...
    if (serialize_buffer.length > kSerializationBufferSize) {
        debug_print("`serialize_buffer.length` can't be greater than `kSerializationBufferSize`\n");
    }
...
NED_TREE_SITTER_SCANNER_EXPORTS(r, ned::editor::languages::scanners::r)
