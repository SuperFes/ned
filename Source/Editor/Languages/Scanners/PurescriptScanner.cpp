// The purescript external scanner, ported from https://github.com/postsolar/tree-sitter-purescript (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::purescript {
...
static State state_new(Lexer *l, const bool * restrict vs, indent_vec *is) {
  return (State) {
    .lexer = l,
    .symbols = vs,
    .indents = is,
...
static uint32_t column(State *state) {
  return is_eof(state) ? 0 : state->lexer->getColumn(state->lexer);
}
...
static void MARK(char *marked_by, bool needs_free, State *state) {
  ...
  state->lexer->markEnd(state->lexer);
}
#else
#define MARK(s, nf, state) state->lexer->markEnd(state->lexer);
#endif
...
static void consume_until(char *target, State *state) {
  ...
  while (PEEK != 0 && !seq(target, state)) {
    ...
    if (first == PEEK) {
      ...
      char *mark_target = scanner_calloc(strlen(prefix) + strlen(target) + 1, 1);
...
#endif
    state->lexer->resultSymbol = result.sym;
    return true;
  } else return false;
}
 ...
 */
void *Create() {
  void *res = scanner_calloc(sizeof(indent_vec), 1);
  return res;
}
 ...
 */
static bool Scan(void *indents_v_, Lexer *lexer, const bool *syms) {
    VoidPtr indents_v{indents_v_};
 ...
 * copied.
 */
static unsigned Serialize(void *indents_v_, char *buffer) {
    VoidPtr indents_v{indents_v_};
  indent_vec *indents = (indent_vec*) indents_v;
 ...
 */
static void Deserialize(void *indents_v_, const char *buffer, unsigned length) {
    VoidPtr indents_v{indents_v_};
  indent_vec *indents = (indent_vec*) indents_v;
 ...
 * Destroy the state.
 */
static void Destroy(void *indents_v_) {
    VoidPtr indents_v{indents_v_};
  indent_vec *indents = (indent_vec*) indents_v;
  VEC_FREE(indents);
  free(indents);
