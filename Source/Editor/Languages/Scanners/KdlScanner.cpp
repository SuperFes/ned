// The kdl external scanner, ported from https://github.com/tree-sitter-grammars/tree-sitter-kdl (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::kdl {
...
void *Create(void) { return scanner_calloc(1, sizeof(Scanner)); }

static void Destroy(void *payload_) {
    VoidPtr payload{payload_}; free(payload); }
...
static bool consume_hashes(Lexer *lexer, uint32_t hash_count) {
...
static bool consume_quotes(Lexer *lexer, unsigned quotes_needed) {
...
static bool scan_raw_start(Scanner *scanner, Lexer *lexer) {
...
static bool scan_raw_body(Scanner *scanner, Lexer *lexer, const bool *valid_symbols) {
...
static bool scan_multi_line_comment(Lexer *lexer) {
    advance(lexer);
    if (lexer->lookahead != '*') return false;
    advance(lexer);
    ...
    lexer->resultSymbol = MULTI_LINE_COMMENT;
    return true;
}
