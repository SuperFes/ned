// The rescript external scanner, ported from https://github.com/rescript-lang/tree-sitter-rescript (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::rescript {
...
void *Create() {
  void* state = scanner_malloc(sizeof(ScannerState));
  memset(state, 0, sizeof(ScannerState));
  return state;
}

static void Destroy(void* state_) {
...
static unsigned Serialize(void* state_, char *buffer) {
    VoidPtr state{state_};
...
static void scan_whitespace(Lexer *lexer, bool skip) {
...
static void skip_whitespace_and_line_comments(Lexer *lexer) {
...
static bool Scan(
  ...
  if (valid_symbols[LIST_CONSTRUCTOR]) {
    lexer->resultSymbol = LIST_CONSTRUCTOR;
    if (lexer->lookahead == 'l') {
      advance(lexer);
      if (lexer->lookahead == 'i') {
        advance(lexer);
  ...
  if (valid_symbols[DICT_CONSTRUCTOR]) {
    lexer->resultSymbol = DICT_CONSTRUCTOR;
    if (lexer->lookahead == 'd') {
      advance(lexer);
      if (lexer->lookahead == 'i') {
        advance(lexer);
  ...
  if (valid_symbols[DECORATOR] && valid_symbols[DECORATOR_INLINE] && lexer->lookahead == '@') {
    ...
    if (is_decorator_start(lexer->lookahead)) {
      ...
      if (lexer->lookahead == '"') {
        ...
        advance(lexer);
        if (is_whitespace(lexer->lookahead)) {
          lexer->resultSymbol = DECORATOR_INLINE;
          lexer->markEnd(lexer);
          return true;
        }
        if (lexer -> lookahead == '(') {
          lexer->resultSymbol = DECORATOR;
          lexer->markEnd(lexer);
          return true;
        }
        return false;
