// The kotlin external scanner, ported from https://github.com/fwcd/tree-sitter-kotlin (src/scanner.c, MIT
...
static inline void stack_push(Stack* stack, char chr, bool triple) {
    if (stack->size >= kSerializationBufferSize)
        abort();
...
static bool scan_string_start(Lexer* lexer, Stack* stack) {
    if (lexer->lookahead != '"')
        return false;
    advance(lexer);
    lexer->markEnd(lexer);
...
static bool scan_string_content(Lexer* lexer, Stack* stack) {
    if (stack->size == 0)
...
static bool scan_multiline_comment(Lexer* lexer) {
    if (lexer->lookahead != '/')
        return false;
    advance(lexer);
    if (lexer->lookahead != '*')
...
static bool scan_whitespace_and_comments(Lexer* lexer) {
    while (iswspace(lexer->lookahead))
        skip(lexer);
    return lexer->lookahead != '/';
}

static bool scan_for_word(Lexer* lexer, const char* word, unsigned len) {
    skip(lexer);
...
static bool scan_automatic_semicolon(Lexer* lexer) {
...
static bool scan_safe_nav(Lexer* lexer) {
...
static bool scan_line_sep(Lexer* lexer) {
...
static bool scan_import_list_delimiter(Lexer* lexer) {
...
void* Create() {
    Stack* stack = scanner_calloc(1, sizeof(Stack));
    if (stack == NULL)
        abort();
    array_init(stack);
    return stack;
