// The php external scanner, ported from https://github.com/tree-sitter/tree-sitter-php (src/scanner.c, MIT
...
static unsigned serialize(Scanner* scanner, char* buffer) {
    ...
    for (unsigned j = 0; j < scanner->heredocs.size; j++) {
        ...
        if (size + 5 + word_size >= kSerializationBufferSize) {
            return 0;
        }
...
static inline bool scan_whitespace(Lexer* lexer) {
    for (;;) {
...
static inline bool is_valid_name_char(Lexer* lexer) {
    return iswalnum(lexer->lookahead) || lexer->lookahead == '_' || lexer->lookahead >= 0x80;
}

static inline bool is_escapable_sequence(Lexer* lexer) {
...
static String scan_heredoc_word(Lexer* lexer) {
...
static inline bool scan_nowdoc_string(Scanner* scanner, Lexer* lexer) {
...
static bool scan_encapsed_part_string(Scanner* scanner, Lexer* lexer, bool is_after_variable, bool is_heredoc,
...
static bool scan(Scanner* scanner, Lexer* lexer, const bool* valid_symbols) {
    ...
    if (valid_symbols[ENCAPSED_STRING_CHARS_AFTER_VARIABLE]) {
        lexer->resultSymbol = ENCAPSED_STRING_CHARS_AFTER_VARIABLE;
    ...
    if (valid_symbols[ENCAPSED_STRING_CHARS]) {
        lexer->resultSymbol = ENCAPSED_STRING_CHARS;
        return scan_encapsed_part_string(scanner, lexer,
                                         /* is_after_variable */ false,
                                         /* is_heredoc */ false,
                                         /* is_execution_string */ false);
    ...
    if (valid_symbols[EXECUTION_STRING_CHARS_AFTER_VARIABLE]) {
        lexer->resultSymbol = EXECUTION_STRING_CHARS_AFTER_VARIABLE;
        return scan_encapsed_part_string(scanner, lexer,
                                         /* is_after_variable */ true,
                                         /* is_heredoc */ false,
    ...
    if (valid_symbols[EXECUTION_STRING_CHARS]) {
        lexer->resultSymbol = EXECUTION_STRING_CHARS;
        return scan_encapsed_part_string(scanner, lexer,
                                         /* is_after_variable */ false,
                                         /* is_heredoc */ false,
                                         /* is_execution_string */ true);
    ...
    if (valid_symbols[ENCAPSED_STRING_CHARS_AFTER_VARIABLE_HEREDOC]) {
        lexer->resultSymbol = ENCAPSED_STRING_CHARS_AFTER_VARIABLE_HEREDOC;
        return scan_encapsed_part_string(scanner, lexer,
                                         /* is_after_variable */ true,
                                         /* is_heredoc */ true,
                                         /* is_execution_string */ false);
    ...
    if (valid_symbols[ENCAPSED_STRING_CHARS_HEREDOC]) {
        lexer->resultSymbol = ENCAPSED_STRING_CHARS_HEREDOC;
        return scan_encapsed_part_string(scanner, lexer,
                                         /* is_after_variable */ false,
                                         /* is_heredoc */ true,
                                         /* is_execution_string */ false);
    ...
    if (valid_symbols[NOWDOC_STRING]) {
        lexer->resultSymbol = NOWDOC_STRING;
...
static inline bool external_scanner_scan(void* payload, Lexer* lexer, const bool* valid_symbols) {
    Scanner* scanner = (Scanner*)payload;
...
static unsigned Serialize(void* p_, char* b) {
    VoidPtr p{p_};
...
static bool Scan(void* p_, Lexer* l, const bool* s) {
    VoidPtr p{p_};
