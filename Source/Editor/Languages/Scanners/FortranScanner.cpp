// The fortran external scanner, ported from https://github.com/stadelmanma/tree-sitter-fortran (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::fortran {
...
static bool skip_literal_continuation_sequence(Lexer *lexer) {
    if (lexer->lookahead != '&') {
        return true;
    }
...
static bool scan_int(Lexer *lexer, int32_t *value, int *count) {
...
static NumberResult scan_number(Lexer *lexer) {
    ...
    if (lexer->lookahead == '.') {
        ...
        if (digits && !iswalnum(lexer->lookahead)) {
            lexer->markEnd(lexer); // add decimal to token
        }
...
static bool scan_boz(Lexer *lexer) {
    lexer->resultSymbol = BOZ_LITERAL;
...
static bool scan_hollerith_constant(Lexer *lexer) {
    ...
    lexer->resultSymbol = HOLLERITH_CONSTANT;
    lexer->markEnd(lexer);
    return true;
}

static bool scan_end_of_statement(Scanner *scanner, Lexer *lexer) {
    ...
    if (lexer->eof(lexer)) {
        skip(lexer);
        lexer->resultSymbol = END_OF_STATEMENT;
        return true;
    }
...
static bool scan_start_line_continuation(Scanner *scanner, Lexer *lexer) {
...
static bool scan_end_line_continuation(Scanner *scanner, Lexer *lexer) {
    if (!scanner->in_line_continuation) {
        return false;
    }
...
static bool scan_string_literal_kind(Lexer *lexer) {
    ...
    lexer->resultSymbol = STRING_LITERAL_KIND;
...
static bool scan_string_literal(Lexer *lexer) {
    ...
    advance(lexer);
    lexer->resultSymbol = STRING_LITERAL;
...
static bool scan_preproc_unary_operator(Lexer *lexer) {
    const char next_char = lexer->lookahead;
    if (next_char == '!' || next_char == '~' || next_char == '-' || next_char == '+') {
        advance(lexer);
        lexer->resultSymbol = PREPROC_UNARY_OPERATOR;
        return true;
    }
    return false;
}
...
static inline bool scan_do_label_eos(Scanner *scanner, Lexer *lexer) {
...
static inline bool scan_do_label_pending(Scanner *scanner, Lexer *lexer) {
...
static void scan_do_label(Scanner *scanner, Lexer *lexer, int32_t label) {
    track_labeled_do(scanner, label);
    lexer->resultSymbol = DO_LABEL;
}

static bool scan_do_label_continue(Scanner *scanner, Lexer *lexer, int32_t label) {
...
static bool scan_label_number_boz(Scanner *scanner, Lexer *lexer, const bool *valid_symbols) {
    ...
    if (result.type == NUMBER_INTEGER) {
        lexer->resultSymbol = INTEGER_LITERAL;
        return true;
    } else if (result.type == NUMBER_FLOAT) {
        lexer->resultSymbol = FLOAT_LITERAL;
        return true;
    }
