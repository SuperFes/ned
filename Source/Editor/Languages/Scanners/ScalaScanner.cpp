// The scala external scanner, ported from https://github.com/tree-sitter/tree-sitter-scala (src/scanner.c, MIT
...
namespace ned::editor::languages::scanners::scala {
...
static unsigned Serialize(void *payload_, char *buffer) {
  ...
  if ((scanner->indents.size + 5) * sizeof(int16_t) > kSerializationBufferSize) {
    return 0;
  }
...
static bool advance_past_blanks(Lexer *lexer) {
  bool found = false;
...
static bool scan_string_content(Lexer *lexer, bool is_multiline, StringMode string_mode) {
  ...
  for (;;) {
    if (lexer->lookahead == '"') {
      ...
      closing_quote_count++;
      if (!is_multiline) {
        lexer->resultSymbol = SINGLE_LINE_STRING_END;
        lexer->markEnd(lexer);
        return true;
      }
      if (closing_quote_count >= 3 && lexer->lookahead != '"') {
        lexer->resultSymbol = MULTILINE_STRING_END;
        lexer->markEnd(lexer);
        return true;
      }
      ...
      switch (string_mode) {
        case STRING_MODE_INTERPOLATED:
          lexer->resultSymbol = is_multiline ? INTERPOLATED_MULTILINE_STRING_MIDDLE : INTERPOLATED_STRING_MIDDLE;
          break;
        case STRING_MODE_RAW:
          lexer->resultSymbol = is_multiline ? RAW_STRING_MULTILINE_MIDDLE : RAW_STRING_MIDDLE;
          break;
        default:
          assert(false);          
      }
      ...
      if (lexer->lookahead == '\\') {
        ...
        if (is_multiline || string_mode == STRING_MODE_RAW) {
          ...
          lexer->resultSymbol = string_mode == STRING_MODE_SIMPLE ? SIMPLE_STRING_MIDDLE : INTERPOLATED_STRING_MIDDLE;
          lexer->markEnd(lexer);
          return true;
        }
...
static bool consume_block_comment_body_ex(Lexer *lexer, bool stop_at_newline) {
  unsigned depth = 1;
  while (depth > 0) {
...
static void consume_block_comment_body(Lexer *lexer) {
...
static bool consume_block_comment_body_on_line(Lexer *lexer) {
  return consume_block_comment_body_ex(lexer, true);
}

static bool finish_block_comment(Lexer *lexer) {
  lexer->markEnd(lexer);
...
static bool lex_block_comment(Lexer *lexer) {
  consume_block_comment_body(lexer);
  return finish_block_comment(lexer);
}

static bool scan_word(Lexer *lexer, const char* const word) {
...
static int read_word(Lexer *lexer, char *buf, int cap) {
  int len = 0;
...
static void skip_blanks_and_block_comments(Lexer *lexer) {
  for (;;) {
...
static bool name_follows_word(Lexer *lexer) {
...
static bool word_is_expression_tail(Lexer *lexer) {
...
static bool modifier_word_allowed(Lexer *lexer) {
...
static bool operand_word_allowed(Lexer *lexer) {
  return !word_is_expression_tail(lexer);
}

static bool modifier_name_follows(Lexer *lexer) {
...
static bool inline_modifier_follows(Lexer *lexer) {
...
static bool is_case_definition_word(Lexer *lexer) {
  advance_past_blanks(lexer);
...
static bool is_case_clause_intro(Lexer *lexer) {
...
static CommentAtLayout check_comment_at_layout(Lexer *lexer,
...
static void skip_delimited(Lexer *lexer, int32_t close, bool escapes) {
...
static int32_t skip_char_or_quote_tail(Lexer *lexer) {
  if (lexer->lookahead == '\\') {
    advance(lexer);
    if (!lexer->eof(lexer) && lexer->lookahead != '\n') {
      advance(lexer);
...
static bool rest_of_line_is_blank_or_comments(Lexer *lexer) {
  for (;;) {
    advance_past_blanks(lexer);
    if (lexer->eof(lexer) || lexer->lookahead == '\n' ||
        lexer->lookahead == '\r') {
...
static bool has_operand(Lexer *lexer) {
  for (;;) {
    if (lexer->eof(lexer)) {
      return false;
    }
...
static bool operand_follows(Lexer *lexer) {
...
static bool is_leading_infix_continuation(Lexer *lexer) {
...
static LineScan scan_rest_of_line(Lexer *lexer) {
  int depth = 0;
...
static bool consume_digit_group(Lexer *lexer, bool *saw_sep) {
  if (lexer->lookahead < '0' || lexer->lookahead > '9') {
    return false;
  }
  advance(lexer);
...
static bool scan_float_with_separator(Lexer *lexer) {
  bool int_sep = false;
  ...
  lexer->markEnd(lexer);
  lexer->resultSymbol = FLOATING_POINT_WITH_SEPARATORS;
  return true;
}
...
static bool scan_impl(void *payload, Lexer *lexer,
                      const bool *valid_symbols);
...
static bool Scan(void *payload_, Lexer *lexer,
  ...
  if (res) {
    if (lexer->resultSymbol == COLON_EOL) {
      scanner->after_colon_eol = 1;
    } else if (lexer->resultSymbol != BLOCK_COMMENT) {
      scanner->after_colon_eol = 0;
    }
  }
  return res;
...
static bool scan_impl(void *payload, Lexer *lexer,
  ...
  #ifdef DEBUG
  {
    if (valid_symbols[ERROR_SENTINEL]) {
      LOG("entering Scan. ERROR_SENTINEL is valid\n");
    } else {
      char debug_str[1024] = "entering Scan valid symbols: ";
  ...
  if (valid_symbols[USING_DIRECTIVE_START] && !valid_symbols[ERROR_SENTINEL] &&
    ...
    lexer->resultSymbol = USING_DIRECTIVE_START;
  ...
  if (valid_symbols[COMMA_OUTDENT] && lexer->lookahead == ',' && prev != -1) {
    ...
    lexer->resultSymbol = COMMA_OUTDENT;
    return true;
  }
  ...
  if (valid_symbols[OUTDENT] && !valid_symbols[ERROR_SENTINEL] &&
      ...
      (lexer->eof(lexer)
           ? scanner->last_column == -1
           : (int16_t)lexer->getColumn(lexer) == scanner->last_column)) {
    lexer->markEnd(lexer);
  ...
  if (
    ...
    switch (check_comment_at_layout(lexer, valid_symbols)) {
      ...
      case COMMENT_SAME_LINE_CODE: {
        ...
        int16_t effective = (int16_t)lexer->getColumn(lexer);
      ...
      case 'e': case 'c': case 'f': case 'y': case 'd': {
        ...
        if (len > 0 &&
          ...
          if (valid_symbols[CONTROL_TAIL_GATE] &&
            ...
            lexer->resultSymbol = CONTROL_TAIL_GATE;
            return true;
          }
          return false;
        }
  ...
  if (valid_symbols[OUTDENT] &&
    ...
    if (lexer->eof(lexer)) {
      ...
      scanner->last_column = (int16_t)lexer->getColumn(lexer);
      scanner->last_char = (int16_t)(lexer->lookahead & 0x7FFF);
    }
  ...
  if (scanner->last_newline_count > 0) {
    ...
    if (
        ...
        (!is_eof && newline_count == 0 &&
         ...
         lexer->getColumn(lexer) == (uint32_t)scanner->last_column)
    ) {
  ...
  if (valid_symbols[END_KEYWORD] && !valid_symbols[ERROR_SENTINEL] &&
    ...
    if (scan_word(lexer, "end") && name_follows_word(lexer)) {
      lexer->markEnd(lexer);
      lexer->resultSymbol = END_KEYWORD;
      return true;
    }
    return false;
  }
  ...
  if (valid_symbols[XML_TAG_START] && !valid_symbols[ERROR_SENTINEL] &&
    ...
    if (is_xml_name_start(lexer->lookahead)) {
      lexer->markEnd(lexer);
      lexer->resultSymbol = XML_TAG_START;
      return true;
    }
    return false;
  }
  ...
  if (!valid_symbols[ERROR_SENTINEL] && is_op_char(lexer->lookahead) &&
    ...
    if (lexer->lookahead == '/' && !shebang &&
      ...
      if (ends_in_slash && lexer->lookahead < 0x80) {
        lexer->markEnd(lexer);
        lexer->resultSymbol = valid_symbols[op_class] ? op_class : OP_NAME;
        return true;
      }
      return false;
    }
    ...
    if (!rest_of_line_is_blank_or_comments(lexer)) {
      ...
      if (is_close_or_separator(lexer->lookahead)) {
        lexer->resultSymbol = postfix_sym;
        return true;
      }
      return false;
    }
  ...
  if (valid_symbols[SIMPLE_STRING_START] && lexer->lookahead == '"') {
    ...
    if (lexer->lookahead == '"') {
      ...
      if (lexer->lookahead == '"') {
        advance(lexer);
        lexer->resultSymbol = SIMPLE_MULTILINE_STRING_START;
        lexer->markEnd(lexer);
        return true;
      }
    }

    lexer->resultSymbol = SIMPLE_STRING_START;
    return true;
  }
