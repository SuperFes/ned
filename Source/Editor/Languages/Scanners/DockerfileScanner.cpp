// The dockerfile external scanner, ported from https://github.com/camdencheek/tree-sitter-dockerfile (src/scanner.c, MIT
...
void* Create() {
    scanner_state* state = scanner_malloc(sizeof(scanner_state));
...
static unsigned Serialize(void* payload_,
    ...
    for (unsigned i = 0; i < state->heredoc_count; i++) {
        ...
        if (pos + len + 1 > kSerializationBufferSize) {
            break;
        }
...
static void Deserialize(void*       payload_,
    ...
    if (length == 0) {
        ...
        for (unsigned i = 0; i < MAX_HEREDOCS; i++) {
            ...
            len++;
            char* heredoc = scanner_malloc(len);
...
static bool scan_marker(scanner_state* state, Lexer* lexer) {
    ...
    if (del_idx == 0) {
        lexer->resultSymbol = HEREDOC_MARKER;
        return true;
    }
    ...
    char* del_copy = scanner_malloc(del_idx + 1);
...
static bool scan_content(scanner_state* state, Lexer* lexer,
    ...
    lexer->resultSymbol = HEREDOC_LINE;
