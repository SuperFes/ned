; Vendored from nvim-treesitter/nvim-treesitter (clojure-and-jank grammar
; follow-up) -- sogaiu/tree-sitter-clojure's own queries/highlights.scm is
; 281 bytes (numbers/strings/comments/quoting operators only, no def-form or
; function-call distinction at all), nowhere near the granularity bar every
; other bundled mode meets, while nvim-treesitter's actively-maintained query
; below is the same ~full-category treatment c.scm/cpp.scm already vendor
; from the same source:
;
;   Source: https://github.com/nvim-treesitter/nvim-treesitter
;   Path:   queries/clojure/highlights.scm
;   Commit: cf12346a3414fa1b06af75c79faebe7f76df080a (master, fetched 2026-08-21
;           -- the same master pin c.scm's own earlier vendoring used)
;   License: Apache-2.0 (see that repo's own LICENSE)
;
; Vendored verbatim below, no changes (unlike c.scm, which appends two custom
; patterns of Ned's own -- nothing here needed adapting). Every node/field
; name it references (sym_lit's name:/namespace: fields, sym_name, sym_ns,
; dis_expr, ...) was verified against sogaiu/tree-sitter-clojure v0.0.13's own
; src/node-types.json directly, not assumed -- an unknown name would throw at
; Query construction time, i.e. at Mode construction. Its predicates
; (#eq?/#any-of?/#lua-match?) are all evaluated by Query.cpp; #set! priority
; is inert there by design (see Query.h's own header comment).
;
; Shared by ClojureMode and JankMode (jank is a Clojure dialect with no
; tree-sitter grammar of its own) -- the kTypeScript/TsxMode sharing pattern.
