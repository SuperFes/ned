...
// rationale -- this is a dedicated key-consuming state machine (Engine.h), the same
...
#ifndef NED_EDITOR_VIM_TYPES_H
#define NED_EDITOR_VIM_TYPES_H
...
// Engine::pendingOperator_), the same way Emacs' own C-u reading isn't a distinct
// InputMode value in BufferView either -- it's not something anything outside the
// engine needs to render differently, beyond the status text Engine already reports.
