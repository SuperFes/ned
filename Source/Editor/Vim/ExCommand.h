...
// only, Engine.h owns execution (mapping a parsed ExCommand onto Buffer::Save/
...
// key-notation parsing (Engine's concern, not this file's) is plain-printable-
// characters-only -- no <Esc>/<C-x> notation.
//

#ifndef NED_EDITOR_VIM_EXCOMMAND_H
#define NED_EDITOR_VIM_EXCOMMAND_H
...
// by the caller (Engine) from its own live state -- this function is otherwise pure
...
// *existing* line the address refers to (Engine's own execution logic is what turns
