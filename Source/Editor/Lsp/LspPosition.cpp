...
LspPosition BytePositionToLsp(const text::Rope& content, std::size_t byteOffset) {
    ...
    while (cursor < byteOffset) {
        const text::Rope::DecodedCodepoint decoded = content.CodepointAt(cursor);
...
std::size_t LspPositionToByte(const text::Rope& content, LspPosition position) {
