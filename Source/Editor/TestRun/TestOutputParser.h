...
//
// Janet-registered parsers are resolved by TestRunner via Config's
...
[[nodiscard]] Outcome ParseCtest(std::string_view output);
...
[[nodiscard]] Outcome ParseCatch2(std::string_view output);
...
[[nodiscard]] Outcome ParsePytest(std::string_view output);
...
[[nodiscard]] Outcome ParseGoTestJson(std::string_view output);
...
[[nodiscard]] Outcome ParseCargoTest(std::string_view output);
...
// standard entities. Typically consumed via Config's results-file
// setting since frameworks write it to a file, not stdout.
[[nodiscard]] Outcome ParseJUnitXml(std::string_view output);
