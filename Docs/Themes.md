...
the accent and fade back over ~200ms. `(ned/set-recency-glow false)` turns it off.

It tints *glyphs* rather than washing the row behind them, so an animation frame disturbs
one or two cells instead of a hundred and sixty -- and since Notcurses emits only what
changed, that is what decides an animation's cost. Its thread is created once and parked on
a condition variable; the render thread never creates, joins or waits on one. Typing
measures the same with it on as off.
