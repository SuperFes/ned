...
// AND (b) either precededByWhitespace or lexer->get_column(lexer) == 0 --
...
static bool scan_emphasis(Scanner *scanner, TSLexer *lexer, const bool *valid_symbols, bool precededByWhitespace) {
    ...
    for (size_t i = 0; i < EMPHASIS_MARKER_COUNT; i++) {
        ...
        const bool notMidWord = precededByWhitespace || lexer->get_column(lexer) == 0;
        ...
        if (wantOpen && !is_emphasis_boundary(lexer->lookahead)) {
            ...
            lexer->result_symbol = marker->open;
            return true;
        }
        if (wantClose && is_emphasis_boundary(lexer->lookahead)) {
            lexer->mark_end(lexer);
            lexer->result_symbol = marker->close;
            return true;
        }
        return false;
    }
...
bool scan(Scanner *scanner, TSLexer *lexer, const bool *valid_symbols) {
    ...
    if (indent_length == 0 && lexer->lookahead == '*') {
        const uint32_t starColumn = lexer->get_column(lexer); // captured before any advance() below -- see the BOLD_OPEN fallback's own use of it
        lexer->mark_end(lexer);
        int16_t stars = 1;
