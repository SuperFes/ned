...
using ned::editor::lsp::PositionToByte;
using ned::editor::lsp::Range;
using ned::editor::lsp::Utf16LengthOfByteRange;
using ned::text::Rope;
using ned::text::RopeStorage;

TEST_CASE("BytePositionToLsp/PositionToByte round-trip over plain ASCII", "[Lsp]") {
    ...
    const Position position   = BytePositionToLsp(content, byteOffset);
    REQUIRE(position.line == 1);
    REQUIRE(position.character == 0);
    REQUIRE(PositionToByte(content, position) == byteOffset);

    // Somewhere mid-line: "second" -> after "sec" is +3 columns/bytes.
    const std::size_t midOffset   = byteOffset + 3;
    const Position midPosition = BytePositionToLsp(content, midOffset);
    REQUIRE(midPosition.line == 1);
    REQUIRE(midPosition.character == 3);
    REQUIRE(PositionToByte(content, midPosition) == midOffset);
}
...
TEST_CASE("BytePositionToLsp counts a multi-byte UTF-8 codepoint as one UTF-16 code unit", "[Lsp]") {
    ...
    const Position position         = BytePositionToLsp(content, byteOffsetOfBang);
    REQUIRE(position.line == 0);
    REQUIRE(position.character == 4); // c, a, f, é -- 4 UTF-16 code units
    REQUIRE(PositionToByte(content, position) == byteOffsetOfBang);
}
...
TEST_CASE("BytePositionToLsp counts a supplementary-plane codepoint as two UTF-16 code units (a surrogate pair)", "[Lsp]") {
    ...
    const Position position      = BytePositionToLsp(content, byteOffsetOfX);
    REQUIRE(position.line == 0);
    REQUIRE(position.character == 2); // one emoji codepoint = 2 UTF-16 code units
    REQUIRE(PositionToByte(content, position) == byteOffsetOfX);
}

TEST_CASE("PositionToByte is bounded by the line's own byte range for an out-of-range character", "[Lsp]") {
    ...
    const std::size_t byteOffset = PositionToByte(content, Position{.line = 0, .character = 999});
    REQUIRE(byteOffset == 3);
}

TEST_CASE("ByteRangeToLspRange computes a single-line ASCII range", "[Lsp]") {
    const std::string content = "abcdef";
    const Range    range   = ByteRangeToLspRange(content, 2, 5); // "cde"
    REQUIRE(range.start == Position{.line = 0, .character = 2});
    REQUIRE(range.end == Position{.line = 0, .character = 5});
}
...
TEST_CASE("ByteRangeToLspRange spans multiple lines", "[Lsp]") {
    ...
    const Range    range     = ByteRangeToLspRange(content, startByte, endByte);
    REQUIRE(range.start == Position{.line = 1, .character = 5}); // "secon|d"
    REQUIRE(range.end == Position{.line = 2, .character = 1});   // "t|hird"
}
...
TEST_CASE("ByteRangeToLspRange counts a non-BMP codepoint as two UTF-16 units", "[Lsp]") {
    ...
    const Range    range   = ByteRangeToLspRange(content, 0, 4); // just the emoji
    REQUIRE(range.start == Position{.line = 0, .character = 0});
    REQUIRE(range.end == Position{.line = 0, .character = 2});
}

TEST_CASE("ByteRangeToLspRange handles startByte == endByte as a zero-width range", "[Lsp]") {
    const std::string content = "abcdef";
    const Range    range   = ByteRangeToLspRange(content, 3, 3);
    REQUIRE(range.start == range.end);
    REQUIRE(range.start == Position{.line = 0, .character = 3});
}
