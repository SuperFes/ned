...
// lines, the disabled scroll bar, the indent guide, the tab glyph --
// structural marks designed to recede, like DarkTheme's own near-background
// 0x3a3a50 border) gets a third of the floor rather than a full skip, so
// "quiet" can never regress to "invisible."
inline void RequireForegroundContrast(const ui::Theme& theme, int floor) {
    ...
    for (const auto& [key, color] : colors) {
        ...
        const bool quietChrome = key == "scroll_bar_disabled_foreground" || key == "border_foreground" ||
                                  key == "indent_guide_foreground" || key == "tab_glyph_foreground";
