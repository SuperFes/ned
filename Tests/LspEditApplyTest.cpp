...
WorkspaceTextEdit Edit(std::size_t startLine, std::size_t startChar, std::size_t endLine, std::size_t endChar, std::string newText) {
    return WorkspaceTextEdit{.start   = Position{.line = startLine, .character = startChar},
...
TEST_CASE("ApplyWorkspaceTextEditsAndRelocate moves an anchor past an earlier insertion", "[EditApply]") {
...
TEST_CASE("ApplyWorkspaceTextEditsAndRelocate leaves an anchor alone for a later edit", "[EditApply]") {
...
TEST_CASE("ApplyWorkspaceTextEditsAndRelocate accounts for a deletion before the anchor", "[EditApply]") {
...
TEST_CASE("ApplyWorkspaceTextEditsAndRelocate relocates across several edits at once", "[EditApply]") {
...
TEST_CASE("ApplyWorkspaceTextEdits still applies every edit as one undo group", "[EditApply]") {
