...
TEST_CASE("Characters already special without a backslash in both flavors pass through unchanged", "[Magic]") {
...
TEST_CASE("Backslash-escaped grouping/alternation/quantifier atoms are unescaped", "[Magic]") {
...
TEST_CASE("Bare grouping/alternation/quantifier characters are escaped to stay literal", "[Magic]") {
    REQUIRE(TranslateVimMagicPattern("(foo)") == "\\(foo\\)");
...
TEST_CASE("Word boundaries translate to \\b", "[Magic]") {
    REQUIRE(TranslateVimMagicPattern("\\<foo\\>") == "\\bfoo\\b");
}

TEST_CASE("Interval quantifiers translate braces and keep the body verbatim", "[Magic]") {
...
TEST_CASE("Non-greedy interval quantifiers translate the leading - to a trailing ?", "[Magic]") {
    REQUIRE(TranslateVimMagicPattern("a\\{-3,5}") == "a{3,5}?");
    REQUIRE(TranslateVimMagicPattern("a\\{-}") == "a*?");
}

TEST_CASE("An empty-body interval translates to a plain star", "[Magic]") {
    REQUIRE(TranslateVimMagicPattern("a\\{}") == "a*");
}

TEST_CASE("Non-capturing groups and \\zs translate", "[Magic]") {
    REQUIRE(TranslateVimMagicPattern("\\%(foo\\)") == "(?:foo)");
    REQUIRE(TranslateVimMagicPattern("foo\\zsbar") == "foo\\Kbar");
}

TEST_CASE("A leading very-magic prefix strips and passes the remainder through unmodified", "[Magic]") {
    REQUIRE(TranslateVimMagicPattern("\\v(foo|bar)+") == "(foo|bar)+");
    REQUIRE(TranslateVimMagicPattern("\\vfoo.*") == "foo.*");
}

TEST_CASE("Grouping/quantifier characters inside a bracket expression are never translated", "[Magic]") {
    REQUIRE(TranslateVimMagicPattern("[(){}|+?]") == "[(){}|+?]");
...
TEST_CASE("An unrecognized backslash escape passes through unchanged", "[Magic]") {
...
TEST_CASE("A translated vim pattern actually compiles and matches via RegexPattern", "[Magic]") {
...
TEST_CASE("A translated interval pattern actually compiles and matches via RegexPattern", "[Magic]") {
...
TEST_CASE("Replacement backreferences translate from vim's \\N form to FormatReplacement's $N", "[Magic]") {
    REQUIRE(TranslateVimMagicReplacement("\\2\\1") == "$2$1");
    REQUIRE(TranslateVimMagicReplacement("\\0") == "$0");
}

TEST_CASE("Bare & is the whole match; \\& is a literal ampersand", "[Magic]") {
    REQUIRE(TranslateVimMagicReplacement("[&]") == "[$&]");
    REQUIRE(TranslateVimMagicReplacement("[\\&]") == "[&]");
}

TEST_CASE("A literal dollar sign is escaped to stay literal for FormatReplacement", "[Magic]") {
    REQUIRE(TranslateVimMagicReplacement("$5") == "$$5");
}

TEST_CASE("A doubled backslash is one literal backslash", "[Magic]") {
    REQUIRE(TranslateVimMagicReplacement("a\\\\b") == "a\\b");
}

TEST_CASE("An unrecognized replacement escape passes through unchanged", "[Magic]") {
