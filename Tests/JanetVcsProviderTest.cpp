...
TEST_CASE("ned/vcs-register-provider registers a provider resolvable via ActiveProviderFor", "[JanetVcsProvider]") {
    ...
    env.DoString(R"(
      (ned/vcs-register-provider "fake"
        {:detect (fn [root] (= root "/repo"))
         ...
         :commit-diff-argv (fn [root commit-hash] ["fake-vcs" "commit-diff" root commit-hash])
         ...
         :reword-commit-argv (fn [root message] ["fake-vcs" "reword-commit" root message])
    ...
    const auto commitDiffArgv = provider->CommitDiffArgv("/repo", "abc1234");
    REQUIRE(commitDiffArgv.argv == std::vector<std::string>{"fake-vcs", "commit-diff", "/repo", "abc1234"});
    ...
    REQUIRE(provider->RewordCommitArgv("/repo", "reworded message").argv ==
            std::vector<std::string>{"fake-vcs", "reword-commit", "/repo", "reworded message"});
...
TEST_CASE("a provider registered without an operation's callbacks reports it as unsupported", "[JanetVcsProvider]") {
    ...
    REQUIRE_THROWS_WITH(provider->RewordCommitArgv("/root", "msg"), "reword commit not supported by this provider");
    ...
    REQUIRE_THROWS_WITH(provider->CommitDiffArgv("/root", "abc1234"), "commit diff not supported by this provider");
}
